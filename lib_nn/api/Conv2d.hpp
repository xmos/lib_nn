...
namespace nn
{
  ...
  public:
    //Valid only
    //Multiple of 32 input and 16 output channels
    Conv2dDepthwiseValidDirect(
        AbstractKernel::Params *akp,
        DerefInputFn *memcpy,
        MatMulDirectFn *aggregator,
        OT_int8 *ot) : Filter2D_DW(akp, memcpy, aggregator, ot) {}
  };
  ...
  public:
    //Valid only
    //Arbitrary input + output channel count
    Conv2dDepthwiseValidIndirect(
        AbstractKernel::Params *akp,
        ImToColValid *memcpy,
        MatMulInt8 *aggregator,
        OT_int8 *ot, int8_t *scratch) : Filter2D_DW(akp, memcpy, aggregator, ot, scratch) {}
  };
