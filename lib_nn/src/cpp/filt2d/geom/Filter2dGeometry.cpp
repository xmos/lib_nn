...
{
  return this->input == other.input && this->output == other.output && this->window == other.window;
}
...
padding_t Filter2dGeometry::Padding() const
{
  ...
  auto t = window.start.row + (output.height-1)*window.stride.row + (window.shape.height-1)*window.dilation.row;
  auto s = window.start.col + (output.width-1)*window.stride.col + (window.shape.width-1)*window.dilation.col;
