...
static void compute_rshift_scale(
      int16_t& rshift,
      int16_t& scale,
      ...
      const int32_t output_zero_point)
{
  ...
  if(offset_scale != 0)
    offset = int16_t(roundf(raw_offset / offset_scale));

  return Conv2dChannelParams {
          .shift1 = shift_pre,
          .scale = scale,
...
{
  return is_depthwise? sumConv2dWeights_Depthwise(filter, kernel_weights, output_channel)
                     : sumConv2dWeights_Deep(filter, kernel_weights, output_channel);
}

/**
 *  For non-depthwise Conv2d's the biases should be the original (e.g. TFlite) biases plus
 *  the input zero-point multiplied by the sum of weights for that output channel.
 * 
 *  For depthwise Conv2d's the biases should be the original (e.g. TFlite) biases plus
 *  the input zero-point multiplied by the sum of weights for that input/output channel.
 * 
        ...
        const bool is_depthwise)
{
  ...
  const unsigned cog_count = (filter.output.depth + VPU_INT8_ACC_PERIOD - 1) >> VPU_INT8_ACC_PERIOD_LOG2;
  ...
  for(unsigned cout = 0; cout < filter.output.depth; ++cout){
    ...
    int32_t bias = computeConv2dBias(filter, biases_in, kernel_weights, input_zero_point, cout, is_depthwise);
    ...
    const int32_t output_zero_point)
{
  ...
  for(unsigned cout = 0; cout < filter.output.depth; ++cout){
    ...
    Conv2dChannelParams params = computeConv2dChannelParams(effective_output_multiplier[cout], output_zero_point);
