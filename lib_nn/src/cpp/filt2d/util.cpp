...
ImageVect ImageVect::add(int const rows, int const cols, int const chans) const
{
  return ImageVect(this->row + rows, this->col + cols, this->channel + chans);
}

ImageVect ImageVect::sub(int const rows, int const cols, int const chans) const
{
  return ImageVect(this->row - rows, this->col - cols, this->channel - chans);
}
...
{
  if (row < start.row || row >= (start.row + shape.height))
    return false;
  if (col < start.col || col >= (start.col + shape.width))
    return false;
...
int ImageRegion::PixelCount() const
{
  return shape.height * shape.width;
}

int ImageRegion::ElementCount() const
{
