...
std::tuple<std::vector<int>, std::vector<int>> OutputTransformFnInt8_Channelwise::Quantizer::solve_for_constraints(MulsAndBias &activationParams, int vlmul_shr,
  ...
  for (auto activationParam : activationParams) {
    ...
    if (!product_range_defined) {
      // As B is constant for all values, set A as maximum valid value and later A is adjusted
      // to fit constraints
      A = 0;
...
void nn::OutputTransformFn::ActivationParams::backprop_output_clamps_to_accu_limits(bool verbose) {
    ...
    double hi =
        ((double)INT8_MAX - bias) / multiplier;
    double lo =
        ((double)INT8_MIN - bias) / multiplier;
    ...
    if (accu_meaningful_max < (int64_t)accu_max_val) {
      ...
      if (accu_meaningful_max < (int64_t)accu_min_val) {
        ...
        int32_t singular_output_value = std::max(
            std::min((int32_t)INT8_MAX, (int32_t)original_bias),
            (int32_t)INT8_MIN);
