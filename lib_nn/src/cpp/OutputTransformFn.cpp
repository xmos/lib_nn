    ...
    const unsigned bits)
{
  ...
  return (input > max_val) ? max_val : (input < min_val) ? min_val
                                                         : input;
}
...
template <class activationT>
std::tuple<int, int> solve_for_constraint(
    ...
    std::vector<int32_t> &accu_max)
{
  ...
  while (trying)
  {
    ...
    for (auto ch = 0; ch < ch_count; ++ch)
    {
      ...
      // bias*(2**(A+B)) fit in 31 bits (must be representable by 16bit * (1<<x))
      // (accu*2**A)*(mul*2**B) fit in 32 bits
      // (accu*2**A)*(mul*2**B) + bias*(2**(A+B)) fit in 32 bits

      int64_t accu_max_16 = shl(accu_max[ch], A);
      int64_t accu_min_16 = shl(accu_min[ch], A);
      if (!check_val_fits(accu_max_16, 16) || !check_val_fits(accu_min_16, 16))
      {
        A--;
        accu_sig_bits--;
        trying = true;
      ...
      if (!check_val_fits(bias_32, 30) ||
          !check_val_fits(prod_max, 32) ||
          !check_val_fits(prod_min, 32) ||
          !check_val_fits(sum_max, 32) ||
    ...
    std::vector<int32_t> &accu_max)
{
  ...
  for (int ch = 0; ch < ch_count; ch++)
  {
    ...
    accu_max_adjusted[ch] = std::min(accu_max[ch], std::max(round_away_from_zero(accu_actual_max), round_away_from_zero(accu_actual_min)));
    accu_min_adjusted[ch] = std::max(accu_min[ch], std::min(round_away_from_zero(accu_actual_max), round_away_from_zero(accu_actual_min)));
  }

  int A, M;
  std::tie(A, M) = solve_for_constraint(
      output_transform_multiplier,
      output_transform_bias,
      accu_min_adjusted,
      accu_max_adjusted);
  ...
  // std::cout << bias_max_bits << " " << bias_min_bits << " " << mul_max_bits << " " << mul_min_bits << " " << bias_max_bits - bias_min_bits << " " << mul_max_bits - mul_min_bits << std::endl;

  assert(bias_max_bits <= 30); //30 bits is the most we can achieve with this method
  ...
  assert(final_shr >= 0);
  // std::cout << "bias_max_bits: " << bias_max_bits << " " << bias_max_bits - 16 - 8 << " final_shr: " << final_shr << std::endl;
...
extern "C" int8_t *output_transform_fn_impl_asm(const OT_int8::Params *params, int8_t *Y, VPURingBuffer *A, int32_t output_channel_group);

int8_t *output_transform_fn_impl(const OT_int8::Params *params, int8_t *Y, VPURingBuffer *A, int32_t output_channel_group)
{
...
int8_t *OT_int8::output_transform_fn(int8_t *Y, VPURingBuffer *A, int32_t output_channel_group)
{
  ...
  // printf("this->params->otv->bias_multipler %p\n", this->params->otv->bias_multipler);
  // printf("this->params->otv->final_shr %p\n", this->params->otv->final_shr);
  // printf("this->params->otv->accu_shr %p\n", this->params->otv->accu_shr);
...
int8_t *output_transform_fn_impl(const OTBinary_int8::Params *params, int8_t *Y, VPURingBuffer *A, int32_t output_channel_group)
{
...
int8_t *OTBinary_int8::output_transform_fn(int8_t *Y, VPURingBuffer *A, int32_t output_channel_group)
{
  return output_transform_fn_impl(this->params, Y, A, output_channel_group);
}
...
int8_t *OTBinary_bin::output_transform_fn(int8_t *Y, VPURingBuffer *A, int32_t output_channel_group)
{
