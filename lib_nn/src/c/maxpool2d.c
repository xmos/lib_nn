...
static void maxpool2d_adjust_starts(int8_t** Y, const int8_t** X,
...
static void maxpool2d_prepare(const nn_image_params_t* x_params,
                              const nn_image_params_t* y_params,
                              const nn_window_params_t* pooling_window,
                              const nn_window_op_job_params_t* job_params) {
  if (MAXPOOL2D_INIT_ERROR_DETECTION_ENABLE) {
    const unsigned final_row =
        ((pooling_window->start.row + pooling_window->shape.height - 1) +
         (pooling_window->stride.vertical * (y_params->height - 1)));
    const unsigned final_col =
        ((pooling_window->start.column + pooling_window->shape.width - 1) +
...
void maxpool2d_ext_ref(nn_image_t* Y, const nn_image_t* X,
                       const nn_image_params_t* x_params,
                       const nn_image_params_t* y_params,
                       const nn_window_params_t* pooling_window,
                       const nn_window_op_job_params_t* job_params,
                       const nn_maxpool2d_flags_e flags) {
  maxpool2d_adjust_starts(&Y, &X, x_params, y_params, pooling_window,
                          job_params, flags);

  maxpool2d_prepare(x_params, y_params, pooling_window, job_params);

  mem_stride_t stride_X_row, stride_Y_row, stride_window_col, stride_window_row,
  ...
  do {
    ...
    stride_X_cog =
        VPU_INT8_EPV - job_params->size.rows * win_vstride_from_prev_start;
  ...
  for (int cog = 0; cog <= chan_groups; cog++) {
    ...
    for (int out_row = 0; out_row < job_params->size.rows; out_row++) {
      for (int out_col = 0; out_col < job_params->size.cols; out_col++) {
        ...
        for (int pool_row = 0; pool_row < pooling_window->shape.height;
             pool_row++) {
          for (int pool_col = 0; pool_col < pooling_window->shape.width;
               pool_col++) {
...
void maxpool2d_ext(nn_image_t* Y, const nn_image_t* X,
                   ...
                   const nn_window_op_job_params_t* job_params,
                   const nn_maxpool2d_flags_e flags) {
  maxpool2d_ext_ref(Y, X, x_params, y_params, pooling_window, job_params,
                    flags);
}
