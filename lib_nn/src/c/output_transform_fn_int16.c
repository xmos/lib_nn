// Copyright 2023-2026 XMOS LIMITED.
// This Software is subject to the terms of the XMOS Public Licence: Version 1.
#include <stdint.h>

#include "nn_op_helper.h"
#include "output_transform_fn_int16.h"
#include "output_transform_fn_int16_mappings.h"

#define VPU_INT16_EPV 16
#define VPU_INT32_EPV 8

static int min(int a, int b) {
    return a < b ? a : b;
}

static int16_t saturate_to_int16(int32_t value) {
    if (value > INT16_MAX) {
        return INT16_MAX;
    }
    if (value < INT16_MIN) {
        return INT16_MIN;
    }
    return value;
}

static int32_t accumulator_from_halves(int16_t low, int16_t high) {
    return (int32_t)high * (1 << 16) + (uint16_t)low;
}

int16_t *output_transform_fn_int16_impl(int16_t *vDvR,
                                        int32_t *mul_add,
                                        int16_t *output,
                                        uint32_t N) {
    for(uint32_t i = 0; i < N; i++) {
        int32_t multiplier = mul_add[ot_int16_mul_index_used_for_output[i]];
        int32_t adder      = mul_add[ot_int16_add_index_used_for_output[i]];
        int32_t accu       = accumulator_from_halves(vDvR[i], vDvR[i+16]);
        accu               = sat_s32((int64_t)accu + adder);
        int64_t answer     = multiplier * (int64_t) accu;
        answer             = (answer + (INT64_C(1) << 29)) >> 30;
        output[i]          = saturate_to_int16(sat_s32(answer));
    }
    return &output[N];
}

extern void output_transform_fn_int16_impl_asm(int16_t *vDvR,
                                               int32_t *mul_add,
                                               int16_t *output,
                                               uint32_t N);

int16_t *output_transform_fn_int16(otfn_int16_params_t *params,
                                   int16_t *Y,
                                   int16_t *vDvR,
                                   int32_t output_channel_group,
                                   int32_t *mul_add) 
{
    int channel_count = params->output_slice_channel_count - output_channel_group * VPU_INT16_EPV;
    int output_count = min(channel_count, VPU_INT16_EPV);
    mul_add += output_channel_group * VPU_INT32_EPV * 4;

#if defined(NN_USE_REF)
    return output_transform_fn_int16_impl(vDvR, mul_add, Y, output_count);
#else
    output_transform_fn_int16_impl_asm(vDvR, mul_add, Y, output_count);
    return Y + output_count;
#endif  // NN_USE_REF
}
