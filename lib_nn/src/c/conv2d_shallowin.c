...
static void conv2d_shallowin_compute_padding(
  ...
  const int32_t init_padding_right =
      conv_window->start.column + k_array_width - x_params->width;
...
static void conv2d_shallowin_adjust_starts(
    ...
    const nn_image_params_t* x_params, const nn_image_params_t* y_params,
  ...
  *Y = ADDR(*Y, start_Y);
  ...
  if (!(flags & CONV2D_SHALLOWIN_FLAG_SLICED_K)) {
    const int32_t start_BSO =
        (job_params->start.channels / VPU_INT8_ACC_PERIOD);
    const int32_t start_K =
        job_params->start.channels * conv_window->shape.height * VPU_INT8_EPV;
...
#endif
void conv2d_shallowin_prepare(nn_conv2d_shallowin_job_t* job,
  ...
  if (CONV2D_INIT_ERROR_DETECTION_ENABLE) {
    ...
    // The product of the input channels and kernel width must be <=
    // VPU_INT8_EPV
    assert(x_params->channels * conv_window->shape.width <= VPU_INT8_EPV);

    // Start can never be negative
    assert(job_params->start.rows >= 0 && job_params->start.cols >= 0 &&
...
void conv2d_shallowin(nn_image_t* Y, const nn_image_t* X, const nn_tensor_t* K,
                      ...
                      const nn_image_params_t* y_params,
                      const nn_window_params_t* conv_window) {
  nn_conv2d_job_params_t full_job = {
      {0, 0, 0}, {y_params->height, y_params->width, y_params->channels}};

  conv2d_shallowin_ext(Y, X, K, BSO, zero_point, x_params, y_params,
                       conv_window, &full_job, 0);
}
...
void conv2d_shallowin_ext(nn_image_t* Y, const nn_image_t* X,
                          ...
                          const nn_conv2d_shallowin_flags_e flags) {
  conv2d_shallowin_adjust_starts(&Y, &X, &K, &BSO, x_params, y_params,
                                 conv_window, job_params, flags);
  ...
  for (int out_chan = 0; out_chan < job_params->size.channels;
    ...
    for (int out_row = 0; out_row < job_params->size.rows; out_row++) {
      ...
      // TODO: Currently init_padding.right is forced to make the assumption
      // that the actual width of the
      //      kernel is 32/C_in, regardless of the convolution window width with
      //      which the operator was initialized. That means that we'll end up
      //      "requiring padding" when we don't actually have to, because the
      //      elements after the user-supplied convolution window width are
      //      guaranteed (per the contract with the user) to be zeroed out. At
      //      the moment I'm stuffing the user-supplied convolution window width
      //      into the plan struct. I'm pretty sure I can just adjust the
      //      conditions for "requires_padding" below using that value to handle
      //      this.
      ...
      if (cur_chans == VPU_INT8_ACC_PERIOD) {
        if (requires_padding) {
          nn_conv2d_hstrip_shallowin_padded(
              Y, X_cog, K, BSO, conv_window->shape.height,
              conv_window->stride.horizontal, x_params->channels, cur_pad_t,
              cur_pad_b, pad_l, pad_r, job.stride.row.X, y_params->channels,
              job_params->size.cols, zero_point_vec);
        } else {
          nn_conv2d_hstrip_shallowin(
              Y, X_cog, K, BSO, conv_window->shape.height,
              conv_window->stride.horizontal, x_params->channels,
              job.stride.row.X, y_params->channels, job_params->size.cols);
        }
      } else {
        if (requires_padding) {
          nn_conv2d_hstrip_tail_shallowin_padded(
              Y, X_cog, K, BSO, conv_window->shape.height,
              conv_window->stride.horizontal, x_params->channels, cur_pad_t,
              cur_pad_b, pad_l, pad_r, job.stride.row.X, y_params->channels,
              job_params->size.cols, zero_point_vec, C_out_tail);
        } else {
          nn_conv2d_hstrip_tail_shallowin(
              Y, X_cog, K, BSO, conv_window->shape.height,
              conv_window->stride.horizontal, x_params->channels,
              job.stride.row.X, y_params->channels, job_params->size.cols,
              C_out_tail);
