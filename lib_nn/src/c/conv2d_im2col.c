...
void conv2d_im2col_init(nn_conv2d_im2col_plan_t* plan,
  ...
  const mem_stride_t window_start_offset =
  ...
  plan->window.shape.len_col =
      ((conv_window->shape.width * conv_window->shape.height *
            x_params->channels +
        VPU_INT8_EPV - 1) >>
       VPU_INT8_EPV_LOG2)
      << VPU_INT8_EPV_LOG2;
  ...
  const int32_t init_padding_right =
      conv_window->start.column + plan->window.shape.width - x_params->width;

  nn_conv2d_job_params_t full_job = {
      {0, 0, 0}, {y_params->height, y_params->width, y_params->channels}};

  for (int i = 0; i < job_count; i++) {
    const nn_conv2d_job_params_t* params =
        (job_params != NULL) ? &job_params[i] : &full_job;
    nn_conv2d_im2col_job_t* job = &jobs[i];

    // Start can never be negative
    assert(params->start.rows >= 0 && params->start.cols >= 0 &&
    ...
    assert(params->start.channels + params->size.channels <=
           y_params->channels);
    ...
    job->init_padding.top =
        init_padding_top - params->start.rows * plan->window.stride.vertical;
    job->init_padding.left =
        init_padding_left - params->start.cols * plan->window.stride.horizontal;
    job->init_padding.bottom =
        init_padding_bottom + params->start.rows * plan->window.stride.vertical;
    job->init_padding.right =
        init_padding_right +
        params->start.cols * plan->window.stride.horizontal;
    ...
    job->stride.chan_group.K =
        plan->window.shape.kernel_row_elements * VPU_INT8_ACC_PERIOD;
    job->stride.start.K =
        params->start.channels * plan->window.shape.kernel_row_elements;
    ...
    job->stride.row.window =
        (plan->window.stride.vertical) * x_row_bytes  // newline
        -
        plan->channels.X * (job->output.cols *
                            plan->window.stride.horizontal);  // carriage return

    const unsigned job_row_bytes = (job->output.cols) * y_params->channels;

    job->stride.chan_group.Y = VPU_INT8_ACC_PERIOD;
    job->stride.col.Y = y_params->channels;  // TODO should this account for
                                             // multiple bytes per channel?
...
void conv2d_im2col(nn_image_t* Y, const nn_image_t* X, const nn_image_t* COL,
  ...
  for (unsigned output_rows = job->output.rows; output_rows > 0;
       output_rows--) {
    int pad_l = job->init_padding.left;
    int pad_r = job->init_padding.right;

    const int pad_lr_delta =
        plan->window.stride.horizontal * (job->output.cols - 1);
    ...
    for (unsigned output_cols = job->output.cols; output_cols > 0;
         output_cols--) {
      const nn_image_t* patch_X =
          X;  // This points it at the top-left cell of the patch.
      const nn_image_t* C = COL;
      memset((void*)C, 0,
             plan->window.shape
                 .len_col);  // very important if len_col > kernel_row_elements!
      if (requires_padding) {
        int pad = 0;
        int pad_tb = 0;
        for (int h = 0; h < plan->window.shape.height; h++) {
          int p = 0;
          pad_tb =
              (cur_pad_t - h > 0) || (plan->window.shape.height - pad_b) <= h;
          for (int i = 0; i < plan->window.shape.width; i++) {
            pad =
                ((pad_l - i > 0) || (plan->window.shape.width - pad_r) <= i) ||
                pad_tb;
            if (pad) {
        ...
        for (unsigned rows_in_patch = plan->window.shape.height;
             rows_in_patch > 0; rows_in_patch--) {
...
#if !CONFIG_SYMMETRIC_SATURATION_conv2d_im2col
      // VLDR(&vpu, vec_0x80);
      // VSTRPV(&vpu, Y, y_mask);// @tail chicken and egg problem with vec
      // registers
      memset(Y, 0x80, job->output.channels);
#endif

      int mat_col_chunks =
          plan->window.shape.kernel_row_elements % VPU_INT8_EPV == 0
      ...
      for (int m_row_chunk = 0; m_row_chunk < mat_row_chunks; m_row_chunk++) {
        const nn_tensor_t* patch_K =
            ADDR(K, m_row_chunk * job->stride.chan_group.K);
        ...
        for (int m_col_chunk = 0; m_col_chunk < mat_col_chunks; m_col_chunk++) {
          ...
          int sub_mat_row_end =
              m_row_chunk < (mat_row_chunks - 1)
                  ? VPU_INT8_ACC_PERIOD
                  : job->output.channels % VPU_INT8_ACC_PERIOD;
          if (sub_mat_row_end == 0)
            sub_mat_row_end =
                VPU_INT8_ACC_PERIOD;  // todo find cleaner way to unwrap modulus
          const nn_tensor_t* sub_matrix =
              ADDR(patch_K, job->stride.row.K * (sub_mat_row_end));

          // TODO still need to find the best way to add a <=32 byte pad to K
          for (int m_row = 0; m_row < VPU_INT8_ACC_PERIOD - sub_mat_row_end;
               m_row++) {
            VLMACCR(&vpu, COL);
            y_mask >>= 1;
          }  // DUMMY on safe memory to rotate @tail
