...
static unsigned conv2d_depthwise_compute_padding(
  ...
  *right = init_padding_right +
  ...
  const int32_t end_padding_left =
      init_padding_left -
      ((job_params->start.cols + job_params->size.cols - 1) *
       conv_window->stride.horizontal);
  const int32_t end_padding_right =
      init_padding_right +
      ((job_params->start.cols + job_params->size.cols - 1) *
       conv_window->stride.horizontal);

  return ((*left <= 0) && (end_padding_left <= 0) && (*right <= 0) &&
          (end_padding_right <= 0));
}
...
static void conv2d_depthwise_adjust_starts(
  ...
  int32_t start_Y = job_params->start.rows * y_row_bytes +
                    y_params->channels * job_params->start.cols +
                    job_params->start.channels;
...
static void conv2d_depthwise_prepare(nn_conv2d_depthwise_job_t* job,
  ...
  job->stride.chan_group.X =
      VPU_INT8_VLMACC_ELMS -
      x_row_bytes * job_params->size.rows * conv_window->stride.vertical;
  job->stride.chan_group.Y =
      VPU_INT8_VLMACC_ELMS - y_row_bytes * job_params->size.rows;
}
...
void conv2d_depthwise(int8_t* Y, const int8_t* X, const int8_t* K,
  ...
  conv2d_depthwise_ext(Y, X, K, BSO, zero_point, x_params, y_params,
...
void conv2d_depthwise_ext(int8_t* Y, const int8_t* X, const int8_t* K,
                          ...
                          const nn_conv2d_depthwise_flags_e flags) {
  conv2d_depthwise_adjust_starts(&Y, &X, &K, &BSO, x_params, y_params,
  ...
  init_padding.unpadded_lr = conv2d_depthwise_compute_padding(
      &init_padding.top, &init_padding.bottom, &init_padding.left,
      &init_padding.right, x_params, conv_window, job_params);

  for (int out_chan = 0; out_chan < job_params->size.channels;
       out_chan += VPU_INT8_VLMACC_ELMS) {
    ...
    for (int out_row = 0; out_row < job_params->size.rows; out_row++) {
      ...
      if (init_padding.unpadded_lr && cur_pad_t == 0 && cur_pad_b == 0) {
        nn_conv2d_hstrip_depthwise(Y, X, K, BSO, conv_window->shape.height,
                                   conv_window->shape.width, x_params->channels,
                                   k_channels, job.stride.row.X,
                                   job.stride.col.window, y_params->channels,
                                   job_params->size.cols, cur_chans);
      } else {
        nn_conv2d_hstrip_depthwise_padded(
            Y, X, K, BSO, conv_window->shape.height, conv_window->shape.width,
