...
static inline int matches_2x2_impl(const nn_window_params_t* pooling_window) {
...
static void avgpool2d_calc_scale_shift(
    nn_avgpool2d_job_t* job, const nn_window_params_t* pooling_window) {
  const unsigned window_pixels =
      pooling_window->shape.height * pooling_window->shape.width;
...
static void avgpool2d_adjust_starts(int8_t** Y, const int8_t** X,
                                    const nn_image_params_t* x_params,
  ...
  const int32_t job_start_row_x =
      pooling_window->start.row +
      pooling_window->stride.vertical * job_params->start.rows;
  const int32_t job_start_col_x =
      pooling_window->start.column +
      pooling_window->stride.horizontal * job_params->start.cols;
  const int32_t start_X = job_start_row_x * x_row_bytes +
                          x_params->channels * job_start_col_x +
...
static void avgpool2d_prepare(nn_avgpool2d_job_t* job,
                              const nn_image_params_t* x_params,
                              const nn_image_params_t* y_params,
                              const nn_window_params_t* pooling_window,
                              const nn_window_op_job_params_t* job_params) {
  if (AVGPOOL2D_INIT_ERROR_DETECTION_ENABLE) {
    ...
    // NOTE: unlike most operators, this one has an output channel group size of
    // 32
  ...
  job->stride.X.row =
      x_row_bytes - pooling_window->shape.width * x_params->channels;
  ...
  job->stride.window.row = win_vstride_from_prev_start -
                           win_hstride_from_prev_start * job_params->size.cols;

  // For a channel group stride, move back to start of job and add 32
  job->stride.X.cog =
      VPU_INT8_ACC_PERIOD - job_params->size.rows * win_vstride_from_prev_start;
  job->stride.Y.cog = VPU_INT8_ACC_PERIOD - job_params->size.rows * y_row_bytes;
}

void avgpool2d_gen_ref(int8_t* Y, const int8_t* X,
                       const channel_count_t image_chans,
                       const nn_window_params_t* pooling_window,
                       const nn_window_op_job_params_t* job_params,
                       const nn_avgpool2d_flags_e flags,
                       const nn_avgpool2d_job_t* job) {
  const unsigned channel_groups =
  ...
  for (unsigned chn_grp = 0; chn_grp <= channel_groups; chn_grp++) {
    ...
    if (chn_grp == channel_groups)
      iter_chans = job_params->size.channels -
                   (channel_groups << VPU_INT8_ACC_PERIOD_LOG2);

    if (iter_chans == 0) break;
    ...
    for (unsigned out_row = 0; out_row < job_params->size.rows; out_row++) {
      for (unsigned out_col = 0; out_col < job_params->size.cols; out_col++) {
        ...
        for (unsigned w_rows = 0; w_rows < pooling_window->shape.height;
             w_rows++) {
          for (unsigned w_cols = 0; w_cols < pooling_window->shape.width;
               w_cols++) {
...
void avgpool2d_2x2_ref(int8_t* Y, const int8_t* X,
                       const channel_count_t image_chans,
                       const nn_window_params_t* pooling_window,
                       const nn_window_op_job_params_t* job_params,
                       const nn_avgpool2d_flags_e flags,
...
void avgpool2d(int8_t* Y, const int8_t* X, const nn_image_params_t* x_params,
               const nn_image_params_t* y_params,
               const nn_window_params_t* pooling_window) {
  nn_window_op_job_params_t full_job = {
      {0, 0, 0}, {y_params->height, y_params->width, x_params->channels}};

  avgpool2d_ext(Y, X, x_params, y_params, pooling_window, &full_job,
                AVGPOOL2D_FLAG_NONE);
}

void avgpool2d_ext(int8_t* Y, const int8_t* X,
                   const nn_image_params_t* x_params,
                   const nn_image_params_t* y_params,
                   const nn_window_params_t* pooling_window,
                   const nn_window_op_job_params_t* job_params,
                   const nn_avgpool2d_flags_e flags) {
  avgpool2d_adjust_starts(&Y, &X, x_params, y_params, pooling_window,
                          job_params, flags);
  ...
  if (matches_2x2_impl(pooling_window)) {
    avgpool2d_2x2(Y, X, x_params->channels, pooling_window, job_params, flags,
                  &job);
  } else {
    avgpool2d_calc_scale_shift(&job, pooling_window);
    avgpool2d_gen(Y, X, x_params->channels, pooling_window, job_params, flags,
                  &job);
  }
}
