...
static void conv2d_1x1_adjust_starts(
  ...
  const uint32_t start_pix =
      job_params->start.rows * y_params->width + job_params->start.cols;

  const int32_t start_X = start_pix * x_params->channels;
  const int32_t start_Y =
      start_pix * y_params->channels + job_params->start.channels;

  *X = ADDR(*X, start_X);
  *Y = ADDR(*Y, start_Y);

  if (!(flags & CONV2D_1X1_FLAG_SLICED_K)) {
    const uint32_t start_cog =
        (job_params->start.channels / VPU_INT8_ACC_PERIOD);

    const int32_t start_K =
        start_cog * VPU_INT8_ACC_PERIOD * x_params->channels;
...
static void conv2d_1x1_prepare(const nn_image_params_t* x_params,
...
void conv2d_1x1(nn_image_t* Y, const nn_image_t* X, const nn_tensor_t* K,
                ...
                const nn_image_params_t* y_params) {
  nn_conv2d_1x1_job_params_t full_job = {
      {0, 0, 0}, {y_params->height * y_params->width, y_params->channels}};

  conv2d_1x1_ext(Y, X, K, BSO, x_params, y_params, &full_job, 0);
}

void conv2d_1x1_ext_ref(nn_image_t* Y, const nn_image_t* X,
                        const nn_tensor_t* K, const nn_bso_block_t* BSO,
                        const nn_image_params_t* x_params,
                        const nn_image_params_t* y_params,
                        const nn_conv2d_1x1_job_params_t* job_params,
                        const nn_conv2d_1x1_flags_e flags) {
  conv2d_1x1_adjust_starts(&Y, &X, &K, &BSO, x_params, y_params, job_params,
                           flags);

  conv2d_1x1_prepare(x_params, y_params, job_params);

  const uint32_t C_out_groups =
      job_params->size.channels >> VPU_INT8_ACC_PERIOD_LOG2;
  ...
  // Stride required to move to next channel input group for current output
  // group in the kernel tensor
  const mem_stride_t cig_stride_body =
      (VPU_INT8_ACC_PERIOD - 1) * x_params->channels + VPU_INT8_EPV;
  const mem_stride_t cig_stride_tail =
      (C_out_tail - 1) * x_params->channels + VPU_INT8_EPV;
  ...
  for (int cog = 0; cog <= C_out_groups; cog++) {
    ...
    // This stride moves K from the first row of the current output group to the
    // final row of the
    // current output group, plus one C_in group (32 channels). It moves K to
    // where it needs to be after processing all output channels (of the current
    // group) on a given input channel group.
    const int32_t cig_stride =
        (cog < C_out_groups) ? cig_stride_body : cig_stride_tail;

    // K currently points to the beginning of the current output group (e.g. 0
    // mod 16). Because the channels within the channel group are processed in
    // reverse order, we need to move it to the final channel of the current
    // group
    K = ADDR(K, cig_stride - VPU_INT8_EPV);

    // Number of channels in the current ouput group (16 unless it's the tail)
    const unsigned cog_chans =
        (cog < C_out_groups) ? VPU_INT8_ACC_PERIOD : C_out_tail;
    ...
    for (int pix = 0; pix < job_params->size.pixels; pix++) {
      ...
      for (int k = 0; k < VPU_INT8_ACC_PERIOD; k++) {
        acc64[k] =
            (BSO->bias_hi[k] << VPU_INT8_ACC_VR_BITS) | (BSO->bias_lo[k] << 0);
      }
      ...
      for (unsigned cig = 0; cig <= C_in_groups; cig++) {
        ...
        const unsigned cig_chans = (cig < C_in_groups)
                                       ? VPU_INT8_EPV
                                       : x_params->channels % VPU_INT8_EPV;
        ...
        for (int k = cog_chans - 1; k >= 0; k--) {
          ...
          // After each VLMACCR (except the last), move K up one row (out
          // channel).
        ...
        // Move K to final row of current output group, then over one input
        // group.
        K = ADDR(K, cig_stride);
      }

      // After processing the final input channel group, if there was an input
      // tail
      // K will be pointing (32-C_in_tail) bytes into the next output channel
      // group. Subtracting that will make K point right at the beginning of the
      // next output channel group, which is where it would be pointing if there
      // was no input tail.
      if (C_in_tail) {
      ...
      // After going through all the input channel groups (and tail), K should
      // be pointing at the start of the next channel group. Subtracting C_in
      // moves it back to pointing at the start of the final channel for the
      // current channel output group, which is where it needs to be for the
      // next pixel.
    ...
    // After each output pixel we move K to point back at the start of the final
    // output channel for that channel output group. The channel output group
    // loop expects it to be pointing at the start of the channel group to be
    // processed. Incrementing K by C_in moves it down one row in the kernel
    // matrix.
    K = ADDR(K, ((int)x_params->channels));
