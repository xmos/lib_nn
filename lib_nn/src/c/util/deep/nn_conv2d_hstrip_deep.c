...
void nn_conv2d_hstrip_deep_ref(
    ...
    const unsigned K_h_stride, const channel_count_t C_in,
...
void nn_conv2d_hstrip_deep_padded_ref(
    nn_image_t* Y, const nn_image_t* X, const nn_tensor_t* K,
    const nn_bso_block_t* BSO, const unsigned K_h, const unsigned K_w,
    const unsigned K_h_stride, const channel_count_t C_in, const unsigned pad_t,
    const unsigned pad_b, const int pad_l_initial, const int pad_r_initial,
    const mem_stride_t x_v_stride, const mem_stride_t k_cout_stride,
    const mem_stride_t y_h_stride, const unsigned out_cols,
  ...
  if (pad_b) {
    ...
    const nn_image_t* K_tmp =
        ADDR(K, patch_rows * K_w * (C_in_groups * VPU_INT8_EPV + C_in_tail));
...
void nn_conv2d_hstrip_tail_deep_padded_ref(
    ...
    const mem_stride_t x_v_stride, const mem_stride_t k_cout_stride,
    const mem_stride_t y_h_stride, const unsigned out_cols,
    const int8_t* zero_point_vec, const channel_count_t C_out_tail) {
  xs3_vpu vpu;
...
void nn_conv2d_hstrip_deep_padded(
  ...
  nn_conv2d_hstrip_deep_padded_ref(Y, X, K, BSO, K_h, K_w, K_h_stride, C_in,
                                   ...
                                   x_v_stride, k_cout_stride, y_h_stride,
                                   out_cols, zero_point_vec);
}
...
void nn_conv2d_hstrip_tail_deep_padded(
    ...
    const mem_stride_t y_h_stride, const unsigned out_cols,
    const int8_t* zero_point_vec, const channel_count_t C_out_tail) {
  nn_conv2d_hstrip_tail_deep_padded_ref(
      Y, X, K, BSO, K_h, K_w, K_h_stride, C_in, pad_t, pad_b, pad_l_initial,
      pad_r_initial, x_v_stride, k_cout_stride, y_h_stride, out_cols,
