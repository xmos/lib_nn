...
TEST_P(ImageGeometry_Test, ByteCounts) {
  auto img = GetParam();
  ASSERT_EQ(img.depth * img.element_bits / CHAR_BIT, img.PixelBytes());
  ASSERT_EQ(img.depth * img.width * img.element_bits / CHAR_BIT, img.RowBytes());
  ASSERT_EQ(img.depth * img.height * img.element_bits / CHAR_BIT, img.ColBytes());
  ASSERT_EQ(img.depth * img.width * img.height * img.element_bits / CHAR_BIT,
            img.ImageBytes());
}
...
TEST_P(ImageGeometry_Test, GetStride) {
  ...
  for (int k = 0; k < 10; ++k) {
    ...
    ASSERT_EQ(stride*CHAR_BIT, (delta.channel + delta.col * img.depth +
...
TEST_P(ImageGeometry_Test, Element) {
  switch (GetParam().element_bits/CHAR_BIT) {
    case 1:
