...
TEST(ImageRegion_Test, Constructor)
{
  ...
  for(int iter = 0; iter < ITER_COUNT; iter++){
    ...
    const ImageGeometry image = {rng.rand<int>(10, 100), 
                                 rng.rand<int>(10, 100), 
                                 rng.rand<int>(10, 100)};
...
TEST(ImageRegion_Test, EndVect)
{
  ...
  for(int iter = 0; iter < ITER_COUNT; iter++){
    ...
    ImageRegion region({start.row, start.col, start.channel},
                       {end.row-start.row+1, end.col-start.col+1, end.channel-start.channel+1});
...
TEST(ImageRegion_Test, ChannelOutputGroups)
{
  ...
  for(int iter = 0; iter < ITER_COUNT; iter++){
    ...
    const auto expected_count = (region.shape.depth + chans_per_group - 1) / chans_per_group;
