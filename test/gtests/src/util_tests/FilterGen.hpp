...
namespace nn {
  ...
  /**
   * Generate a vector of Filter2dGeometry objects that require padding for testing.
   */
  std::vector<Filter2dGeometry>
  PaddedFilters(int Y_dim = 3, int K_dim = 3)
  {
  ...
  /**
   * Generate a vector of Filter2dGeometry objects that use dilation, but don't require any padding
   */
  std::vector<Filter2dGeometry>
  DilatedFilters(int Y_dim = 3, int K_dim = 3)
  {
          ...
          for(int D_w = 2; D_w <= 3; D_w++){
            ...
            for(int Y_h = 1; Y_h <= Y_dim; Y_h++){
              for(int Y_w = 1; Y_w <= Y_dim; Y_w++){
                ...
                int last_start_row = win.start.row + (Y_h-1) * win.stride.row + (K_h-1) * win.dilation.row;
                int last_start_col = win.start.col + (Y_w-1) * win.stride.col + (K_w-1) * win.dilation.col;
