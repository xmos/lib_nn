...
namespace {
  ...
  static inline int last_x_dim(int out_index, int window_dim, int dilation, int start, int stride)
  {
    return start + out_index * stride + (window_dim - 1) * dilation;
  }


  static inline bool IsValidGeometry(nn::Filter2dGeometry& f)
  {
    if( int(f.input.height)  <= 0) return false;
    if( int(f.input.width)   <= 0) return false;
    if( int(f.output.height) <= 0) return false;
    if( int(f.output.width)  <= 0) return false;
    return true;
  }


  static inline void CompleteFilter(nn::Filter2dGeometry& f)
  {
    f.input.height = last_x_dim(f.output.height-1, f.window.shape.height, 
                          f.window.dilation.row, f.window.start.row, f.window.stride.row);
    f.input.width  = last_x_dim(f.output.width-1, f.window.shape.width, 
                          f.window.dilation.col, f.window.start.col, f.window.stride.col);
  }


  static inline void SubUpdate(nn::Filter2dGeometry& f)
  {
    #define UPDATE(FIELD, MIN, MAX, STEP)   do {      \
      FIELD += STEP;                                  \
      if(FIELD <= MAX) return;                        \
      FIELD = MIN; } while(0)

    UPDATE(f.window.dilation.col, 1, 3, 1);
    UPDATE(f.window.dilation.row, 1, 3, 1);
    UPDATE(f.window.stride.col,   1, 3, 1);
    UPDATE(f.window.stride.row,   1, 3, 1);
    UPDATE(f.window.shape.depth,  4, f.input.depth, 4);
    UPDATE(f.window.shape.width,  1, f.input.width, 1);
    UPDATE(f.window.shape.height, 1, f.input.height, 1);
    UPDATE(f.output.width,        1, 8, 1);
    UPDATE(f.output.height,       1, 8, 1);
    UPDATE(f.input.depth,         4, 16, 4);

    #undef UPDATE

    // If it made it here, we've gone through everything.
    f = nn::FilterGeometryIterator::END;
  }


  
  static void Update(nn::Filter2dGeometry& filter){
    do {
      SubUpdate(filter);
      if(filter == TestFilterGeometryIterator::END) return;
    } while(!IsValidGeometry(filter));
  }


class Test_Im_to_col_valid2: public ::testing::TestWithParam<nn::Filter2dGeometry> {};

TEST_P(Test_Im_to_col_valid2, BasicTest)
{
  auto geom = GetParam();
  ...
  ImageParams X(geom.input.height, geom.input.width, geom.input.depth, 8); //8 bits per elemnet
  WindowGeometry K (geom.window.shape.height, geom.window.shape.width, geom.window.stride.col, 
                    geom.window.stride.row, geom.window.dilation.col, geom.window.dilation.row);

  Im_to_col_valid cpy(X, K, geom.window.shape.depth);
  ...
  auto T = std::vector<int8_t>(scratch_bytes);
  auto expected = std::vector<int8_t>(scratch_bytes);
  auto X_mem = std::vector<int8_t>(geom.input.imageElements() + overread_bytes);

  auto window_start_cov = nn::AddressCovector<int8_t>( geom.window.stride.row * geom.input.rowBytes(),
                                                               geom.window.stride.col * geom.input.depth, 1);
  auto window_elm_cov = nn::AddressCovector<int8_t>( geom.input.rowBytes() * geom.window.dilation.row, 
                                                             geom.input.depth * geom.window.dilation.col, 1);

  //The count of x channels from whick a memcopy could start 
  int output_ch_starts = geom.input.depth / geom.window.shape.depth;

  for(int output_h = 0; output_h < geom.output.height; ++output_h){
    for(int output_w = 0 ; output_w < geom.output.width; ++output_w){
      for(int output_c = 0; output_c < output_ch_starts; output_c += 4){ //only test from aligned memory addresses

        auto win_start = window_start_cov.resolve(&X_mem[0], output_h, output_w, output_c);

        for(auto j = 0; j < sizeof(int8_t) * X_mem.size(); ++j)
          X_mem[j] = rng.rand<int8_t>();

        std::memset(&T[0], 0x55, sizeof(int8_t) * T.size());
        
        cpy.memcopy_fn(&T[0], &X_mem[0], output_h, output_w, output_c); 

        int t_idx = 0;
      
        for(int kh = 0; kh < geom.window.shape.height; ++kh){
          for(int kw = 0 ; kw < geom.window.shape.width; ++kw){
            for(int kc = 0 ; kc < geom.window.shape.depth; ++kc){ 
              expected[t_idx++] = window_elm_cov.resolve(win_start, kh, kw, kc)[0];
            }
          }
        }
...
auto iter = TestFilterGeometryIterator( nn::Filter2dGeometry(
                                            nn::ImageGeometry(1,1,1),
                                            nn::ImageGeometry(1,1,1),
                                            nn::WindowGeometry(1,1,4,  0,0,  1,1,0, 1,1)), Update ); 
INSTANTIATE_TEST_SUITE_P(,Test_Im_to_col_valid2 , ::testing::ValuesIn(iter.begin(), iter.end()));
