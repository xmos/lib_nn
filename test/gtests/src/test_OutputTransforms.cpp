...
namespace nn
{
      ...
      std::vector<int32_t> &accu_max)
  {
    ...
    {
      int32_t abs_max_accu = std::max(std::abs(accu_min[ch]), std::abs(accu_max[ch]));

      double r = std::pow(2.0, std::abs(rng.rand<double>()) * multiplier_dynamic_range) /
  ...
  void pick_accu_range(std::vector<int32_t> &accu_min, std::vector<int32_t> &accu_max)
  {
        ...
        for (int ocg = 0; ocg < ocg_count; ++ocg)
        {
          ...
          for (int t = 0; t < 1 << 12; t++)
          {
            ...
            for (int output_chan = 0; output_chan < chs_in_group; ++output_chan)
            {
              ...
              double f_expected = std::round(std::min(std::max(expected, (double)INT8_MIN), (double)INT8_MAX));
              EXPECT_NEAR((int)f_expected, (int)Y[actual_output_channel], 1) << expected << " actual_output_channel: " << actual_output_channel << " output_ch_count: " << output_ch_count << " accu_value: " << v << " f_biases: " << f_biases[actual_output_channel] << " f_multipliers: " << f_multipliers[actual_output_channel] << " accu_max: " << accu_max[actual_output_channel] << " accu_min: " << accu_min[actual_output_channel];
            }
          }
          y = next_y;
        }
