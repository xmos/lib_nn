...
/**
 * Construct and execute a AvgPool2d_Generic operator with the specified
...
static std::vector<int8_t> RunOperator_Generic(
  ...
  nn::AvgPool2d_Generic mp_op(&params.ak_params, &memcopy_handler, &agg_handler,
                              &ot_handler);
...
/**
 * Construct and execute a AvgPool2d_Valid operator with the specified geometry
...
static std::vector<int8_t> RunOperator_Valid(
  ...
  nn::AvgPool2d_Valid mp_op(&params.ak_params, &memcopy_handler, &agg_handler,
                            &ot_handler);
...
/**
 * Test Case -- Compare output of  AvgPool2d_Generic to our reference
 * implementation
 */
TEST(AvgPool2d_Generic_Test, CompareWithReference) {
  ...
  for (auto filter : TestFilterIterator()) {
    ...
    if (filter.Padding().HasPadding())
      continue;  /// TODO: skip these for now because we compute border averages
                 /// differently
    ...
    for (int iter = 0; iter < REPETITIONS; iter++) {
      ...
      auto ref_output =
          nn::test::ops::ref::AveragePoolReference(filter, &input_img[0]);
          ...
          for (int col = 0; col < filter.output.width; col++) {
            for (int chan = 0; chan < filter.output.depth; chan++) {
              ...
              if (delta > 1) {
                ...
                auto mx = loc.Fold<float, int8_t>(
                    &input_img[0],
                    [&](const nn::ImageVect& filter_coords,
                        const nn::ImageVect& input_coords, const float acc,
                        const int8_t element, const bool is_padding) -> float {
                      stream << int(element) << ", ";
                      return acc + (element * div);
                    },
                    0, 0);
                stream << "] * " << int(scale) << ") >> " << shift << " = "
                       << mx;
...
/**
 * Test Case -- Compare output of  AvgPool2d_Valid to our reference
 * implementation
 */
          ...
          for (int col = 0; col < filter.output.width; col++) {
            for (int chan = 0; chan < filter.output.depth; chan++) {
              ...
              if (delta > 1) {
                ...
                auto mx = loc.Fold<int32_t, int8_t>(
                              &input_img[0],
                              [&](const nn::ImageVect& filter_coords,
                                  const nn::ImageVect& input_coords,
                                  const int32_t acc, const int8_t element,
