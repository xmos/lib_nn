...
TEST_F(Conv2dDenseReferenceRegression, BasicTest)
{
  ...
  //   a_mean += i;
  // }
  // a_std /= (INT8_MAX - INT8_MIN);
  // a_mean /= (INT8_MAX - INT8_MIN);
  // std::cout << "a_std " << a_std << std::endl;
                  ...
                  for (int k_h_stride = 1; k_h_stride <= 2; ++k_h_stride)
                  {
                    for (int k_v_stride = 1; k_v_stride <= 2; ++k_v_stride)
                    {
                      ...
                      if (output_height <= 0 || output_width <= 0)
                        continue;
                      // std::cout << " x_height: " << x_height << " x_width: " << x_width << " x_channels: " << x_channels << " k_height: " << k_height << " k_width: " << k_width << " k_depth: " << k_depth << " k_h_dilation: " << k_h_dilation << " k_v_dilation: " << k_v_dilation << " k_h_stride: " << k_h_stride << " k_v_stride: " << k_v_stride << std::endl;
                      ...
                      for (int r = 0; r < 1; r++)
                      {
                        ...
                        //     {
                        //       for (int ki = 0; ki < geom.input.depth; ki++)
                        //       {
                        //         int idx = ko * (geom.window.shape.height * geom.window.shape.width * geom.input.depth) + kh * (geom.window.shape.width * geom.input.depth) + kw * geom.input.depth + ki;
                        ...
                        // for (int xh = 0; xh < geom.input.height; xh++)
                        // {
                        //   for (int xw = 0; xw < geom.input.width; xw++)
                        //   {
                        //     for (int xi = 0; xi < geom.input.depth; xi++)
                        //     {
                        //       int idx = xh * (geom.input.width * geom.input.depth) + xw * geom.input.depth + xi;
                        //       input[idx] = rng.rand<int8_t>();
                        //     }
                        //   }
                        // }
                        ...
                        padding_t padding = {(int16_t)0, (int16_t)0, (int16_t)0, (int16_t)0};
                        ...
                        for (int yh = 0; yh < Y.height; yh++)
                        {
                          for (int yw = 0; yw < Y.width; yw++)
                          {
                            for (int yd = 0; yd < Y.depth; yd++)
                            {
                              ...
                              EXPECT_NEAR((int)expected[idx], (int)output[idx], 1) << "tflite: " << (int)expected[idx] << " xcore: " << (int)output[idx] << " " << eff_mult[yd] << std::endl;
                            }
                          }
                        }
                      }
