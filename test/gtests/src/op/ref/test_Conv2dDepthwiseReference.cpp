 ...
 * 
 */
class Conv2dDepthwiseReferenceTestA : public ::testing::TestWithParam<Filter2dGeometry> {};

TEST_P(Conv2dDepthwiseReferenceTestA, NoPadding)
{
  auto geom = GetParam();
  ...
  int32_t acc32 = geom.window.shape.imagePixels() * ( int32_t( weights[0] ) * ( int32_t( input[0] ) - input_zero ) );
  ...
  auto output = nn::test::ops::ref::Conv2dDepthwiseReference(geom, &input[0], &weights[0], &bias[0], &eff_mult[0],
                                                         input_zero, output_zero);

  ASSERT_EQ(output, expected);
}

INSTANTIATE_TEST_SUITE_P(Basic, Conv2dDepthwiseReferenceTestA, ::testing::Values( Filter2dGeometry(
                                                                            ImageGeometry(1,1,2),
                                                                            ImageGeometry(1,1,2),
...
static auto iterA = nn::test::ParamedRandIter<Filter2dGeometry, SimpleFilter>(100, SimpleFilter(true, false));
INSTANTIATE_TEST_SUITE_P(Random, Conv2dDepthwiseReferenceTestA, ::testing::ValuesIn(iterA.begin(), iterA.end()));
...
class Conv2dDepthwiseReferenceTestB : public ::testing::TestWithParam<Filter2dGeometry> {};

TEST_P(Conv2dDepthwiseReferenceTestB, WithPadding)
{
  auto geom = GetParam();
  ...
  auto pix_acc =  int32_t(weights[0]) * (int32_t(input[0]) - input_zero);
...
static auto iterB = nn::test::ParamedRandIter<Filter2dGeometry, SimpleFilter>(100, SimpleFilter(true, true), 763577);
INSTANTIATE_TEST_SUITE_P(Random, Conv2dDepthwiseReferenceTestB, ::testing::ValuesIn(iterB.begin(), iterB.end()));