...
struct SimpleFilter { 
  ...
  SimpleFilter(bool depthwise = false, bool padded = false, unsigned cog_size = 16) 
    : depthwise(depthwise),
      padded(padded),
      cog_size(cog_size) {}
};
 ...
 */
template<>
inline Filter2dGeometry Rand::rand<Filter2dGeometry, SimpleFilter>(SimpleFilter p)
{
  if(p.padded){
...
static inline int8_t round_int8(float val){
  ...
  return int8_t( std::max<float>( std::numeric_limits<int8_t>::min(), 
                 std::min<float>( std::numeric_limits<int8_t>::max(), r) ) );
}



static inline std::ostream& operator<<(std::ostream &stream, const std::vector<int8_t> &v){
...
static inline std::ostream& operator<<(std::ostream &stream, const std::vector<int32_t> &v){
