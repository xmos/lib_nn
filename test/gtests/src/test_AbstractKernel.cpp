...
namespace nn
{
  ...
  public:
    ...
    MockAbstractKernel(AbstractKernel::Params *kparams, int32_t output_slice_channel_count)
        : AbstractKernel(kparams), output_slice_channel_count(output_slice_channel_count) {}
                ...
                for (int r_width_end = r_width_start + 1; r_width_end <= y_width; ++r_width_end)
                {
                  for (int r_channels_start = 0; r_channels_start < y_channels; ++r_channels_start)
                  {
                    for (int r_channels_end = r_channels_start + 1; r_channels_end <= y_channels; ++r_channels_end)
                    {
                      ...
                      MockAbstractKernel f(&akp, r_channels_end - r_channels_start);
