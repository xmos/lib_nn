...
static void benchmark_conv2d_deep_case(nn_image_params_t* x_params,
  ...
  unsigned K_bytes = y_params->channels * window->shape.height *
                     window->shape.width * x_params->channels;
  unsigned BSO_bytes =
      ((y_params->channels + (VPU_INT8_ACC_PERIOD - 1)) / VPU_INT8_ACC_PERIOD) *
      sizeof(nn_bso_block_t);
...
void benchmark_conv2d_deep(int argc, char** argv) {
  ...
  while (argc >= 12) {
    ...
    nn_window_params_t window = {
        {K_h, K_w}, {start_row, start_col}, {vstride, hstride}};
