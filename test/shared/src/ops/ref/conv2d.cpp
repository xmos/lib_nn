...
std::vector<int8_t> nn::test::ops::ref::TransposeConv2dDenseReference(
  ...
  // printf("filter_geometry.window.stride.row %d filter_geometry.window.stride.col %d\n", filter_geometry.window.stride.row, filter_geometry.window.stride.col);
  ...
  const int num_elements = shape.output.FlatSize();
  std::vector<int32_t> scratch_buffer(num_elements);

  tflite::reference_integer_ops::TransposeConv(
