...
FilterGeometryIterator::FilterGeometryIterator(std::initializer_list<IFilterFrame*> filter_frames)
...
AllUnpadded::AllUnpadded( const nn::Filter2dGeometry max_geometry,
    ...
    : max_geometry(max_geometry), channel_step(channel_step), depthwise(depthwise)
{
}
...
{
#define UPDATE(FIELD, INIT, STEP)    do {                 \
    filter.FIELD += STEP;                                 \
  ...
  if(!depthwise)
    UPDATE(input.depth, channel_step, channel_step);
...
bool AllUnpadded::UpdateFilter()
{
  ...
  // Depthwise filters have window.shape.depth == 1, dense filters have window.shape.depth = input.depth
  ...
  input.height = window.start.row                                   // start row for first output
                 + (output.height - 1) * window.stride.row          // start row for last output
                 + (window.shape.height - 1) * window.dilation.row  // last row for last output
                 + 1;                                               // input height

  input.width  = window.start.col                                   // start col for first output
                 + (output.width - 1) * window.stride.col           // start col for last output
                 + (window.shape.width - 1) * window.dilation.col   // last col for last output
                 + 1;                                               // input width
...
MakePadded::MakePadded( const nn::padding_t max_padding,
                        const bool depthwise)
...
bool MakePadded::_UpdateFilter()
{
  ...
  if( (cur_padding.right <= max_padding.right)
      ...
      && (cur_padding.right + cur_padding.left < filter.input.width)) return true;
  cur_padding.right = 0;

  cur_padding.bottom++;
  if( (cur_padding.bottom <= max_padding.bottom)
      && (cur_padding.bottom < filter.window.shape.height)
      && (cur_padding.bottom + cur_padding.top < filter.input.height) ) return true;
  cur_padding.bottom = 0;

  cur_padding.left++;
  if( (cur_padding.left <= max_padding.left)
      && (cur_padding.left < filter.window.shape.width) ) return true;
  cur_padding.left = 0;

  cur_padding.top++;
  if( (cur_padding.top <= max_padding.top)
      && (cur_padding.top < filter.window.shape.height) ) return true;
...
bool MakePadded::UpdateFilter()
{
  ...
  input.height = (output.height - 1) * window.stride.row          // start row for last output
                 + (window.shape.height - 1) * window.dilation.row  // last row for last output
                 + 1;                                               // input height

  input.width  = (output.width - 1) * window.stride.col           // start col for last output
                 + (window.shape.width - 1) * window.dilation.col   // last col for last output
                 + 1;        
...
nn::Filter2dGeometry nn::ff::MakeUnpaddedDepthwise(nn::Filter2dGeometry filter)
{
