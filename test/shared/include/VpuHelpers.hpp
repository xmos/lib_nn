...
namespace nn {
  ...
  static inline void split_acc32(int16_t& acc_hi, int16_t& acc_lo, const int32_t acc32)
  {
  ...
  static inline int32_t merge_acc32(const int16_t acc_hi, const int16_t acc_lo)
  {
    ...
    return acc32 & int32_t(uint16_t(acc_lo)); // don't let it sign-extend acc_lo. It should be treated as unsigned!
  }
  ...
  static inline T_out vpu_sat(T_in input, bool symmetric = true)
  {
    ...
    const T_out lower_bound = symmetric? -upper_bound : std::numeric_limits<T_out>::min();
