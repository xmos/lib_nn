...
class Rand {
  ...
  protected:
    ...
    // Because functions can't be overloaded on return type only, to overload get_rand() we need to
    // actually pass it a type indicating what we want.
    ...
    // Because we can't just statically cast a std::tuple<> into existence, this overload captures them.
...
std::tuple<Ts...> Rand::get_rand(Tag<std::tuple<Ts...>>, 
                                 ...
                                 std::tuple<Ts...> max)
{
  return std::make_tuple<Ts...>(  (this->rand<Ts>(std::get<indices>(min), 
                                                  std::get<indices>(max)))...);
}
...
std::tuple<Ts...> Rand::get_rand(Tag<std::tuple<Ts...>>, 
                                 ...
                                 std::tuple<Ts...> max)
{
  return get_rand(Tag<std::tuple<Ts...>>(), typename gen_indices<Ts...>::type(), min, max);
}
...
template <> double Rand::get_rand<double>(Tag<double>);
// Uniform distribution over the range [max, max)
template <> float Rand::get_rand<float>(Tag<float>, float, float);
// Uniform distribution over the range [min, max)
template <> double Rand::get_rand<double>(Tag<double>, double, double);
...
class RandIterBase {
  ...
  public:
    RandIterBase(unsigned count) : 
      count(count), rng(Rand()) {}
...
class RandIter : public RandIterBase<T> {
  ...
  protected:
    ...
    virtual T NextRand() override {
      return this->rng.template rand<T>();
    }

  public:
    RandIter(unsigned count) 
      : RandIterBase<T>(count) {}
...
class ParamedRandIter : public RandIterBase<T_out> {
  ...
  public:
    ...
    constexpr ParamedRandIter(unsigned count, const T_param& params, int seed) noexcept 
