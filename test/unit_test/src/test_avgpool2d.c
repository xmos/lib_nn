...
void test_avgpool2d_case0() {
  ...
  for (unsigned v = start_case; v < N_casses && v < stop_case; v++) {
    ...
    memset(Y, 0xCC,
           casse->Y.height * casse->Y.width *
               casse->channels);  // too expensive to write the whole image, so
...
void test_avgpool2d_case1() {
  ...
  const test_case_t casses[] = {
      {{1, 1}, {0, 0}, {0, 0}, __LINE__}, {{2, 2}, {0, 0}, {0, 0}, __LINE__},
      {{3, 3}, {0, 0}, {0, 0}, __LINE__}, {{4, 4}, {0, 0}, {0, 0}, __LINE__},
      {{5, 5}, {0, 0}, {0, 0}, __LINE__}, {{6, 6}, {0, 0}, {0, 0}, __LINE__},
      {{8, 8}, {0, 0}, {0, 0}, __LINE__}, {{12, 12}, {0, 0}, {0, 0}, __LINE__},
      {{1, 2}, {0, 0}, {0, 0}, __LINE__}, {{2, 1}, {0, 0}, {0, 0}, __LINE__},
      {{3, 8}, {0, 0}, {0, 0}, __LINE__}, {{12, 4}, {0, 0}, {0, 0}, __LINE__},
      {{1, 1}, {1, 1}, {0, 0}, __LINE__}, {{1, 1}, {0, 0}, {1, 1}, __LINE__},
  ...
  for (unsigned v = start_case; v < N_casses && v < stop_case; v++) {
    ...
    for (int vpos = 0; vpos < job_params.size.rows; vpos++) {
      for (int hpos = 0; hpos < job_params.size.cols; hpos++) {
        int x_top = casse->x_start.row +
                    (casse->y_start.row + vpos) * window_config.stride.vertical;
        int x_left = casse->x_start.col + (casse->y_start.col + hpos) *
                                              window_config.stride.horizontal;
        ...
        for (int chn = 0; chn < y_params.channels; chn++) {
          ...
          Y_exp[vpos + casse->y_start.row][hpos + casse->y_start.col][chn] =
              avg;
        }
      }
    }
    ...
    avgpool2d_ext((int8_t*)Y, (int8_t*)X, &x_params, &y_params, &window_config,
                  &job_params, AVGPOOL2D_FLAG_NONE);
...
void test_avgpool2d_case3() {
  ...
  const test_case_t casses[] = {
      //  X               //Chans
      // {   2,  2,          16          , __LINE__},  // 0
      {4, 4, 16, __LINE__},   {6, 6, 16, __LINE__},  {8, 8, 16, __LINE__},
      {2, 4, 16, __LINE__},   {4, 2, 16, __LINE__},  // 5
      {8, 6, 16, __LINE__},   {2, 16, 16, __LINE__}, {32, 16, 16, __LINE__},
      {32, 32, 16, __LINE__},

      {2, 2, 32, __LINE__},  // 10
      {2, 2, 48, __LINE__},   {2, 2, 64, __LINE__},  {2, 2, 4, __LINE__},
      {2, 2, 8, __LINE__},    {2, 2, 12, __LINE__},  // 15
      {2, 2, 20, __LINE__},   {2, 2, 24, __LINE__},  {2, 2, 28, __LINE__},
      {2, 2, 36, __LINE__},

      {4, 8, 40, __LINE__},  // 20
      {12, 6, 12, __LINE__},  {16, 2, 40, __LINE__}, {4, 24, 36, __LINE__},
      {32, 32, 60, __LINE__},
  };
  ...
  for (unsigned v = start_case; v < N_casses && v < stop_case; v++) {
    ...
    nn_image_params_t y_params = {casse->height / 2, casse->width / 2,
                                  casse->channels};

    for (int r = 0; r < y_params.height; r++) {
      for (int c = 0; c < y_params.width; c++) {
        for (int ch = 0; ch < y_params.channels; ch++) {
          ((int8_t*)X)[IMG_ADDRESS_VECT(&x_params, 2 * r + 0, 2 * c + 0, ch)] =
              (ch & 1) ? 110 : -110;
          ((int8_t*)X)[IMG_ADDRESS_VECT(&x_params, 2 * r + 1, 2 * c + 0, ch)] =
              (ch & 1) ? 90 : -90;
          ((int8_t*)X)[IMG_ADDRESS_VECT(&x_params, 2 * r + 0, 2 * c + 1, ch)] =
              (ch & 1) ? 120 : -120;
          ((int8_t*)X)[IMG_ADDRESS_VECT(&x_params, 2 * r + 1, 2 * c + 1, ch)] =
              (ch & 1) ? 80 : -80;
        }
      }
    }
...
void test_avgpool2d() {
  ...
  // src/test_avgpool2d.c:49:test_avgpool2d_case1:FAIL: Expected -128 Was -127.
  // Y[0][0][8] was wrong [line 219] RUN_TEST(test_avgpool2d_case1);
