...
void test_conv2d_im2col_case0() {
  nn_tensor_t WORD_ALIGNED
      K[CHANS_OUT][(((K_H * K_W * CHANS_IN + 3) >> 2) << 2)];
  nn_image_t WORD_ALIGNED
      COL[((K_H * K_W * CHANS_IN + VPU_INT8_EPV - 1) >> VPU_INT8_EPV_LOG2)
          << VPU_INT8_EPV_LOG2];
  ...
  nn_conv2d_job_params_t job_params = {{0, 0, 0},
                                       {Y_HEIGHT, Y_WIDTH, CHANS_OUT}};
  conv2d_im2col_init(&plan, &job, &x_params, &y_params, &job_params,
  ...
  conv2d_im2col((nn_image_t*)Y, (nn_image_t*)X, (nn_image_t*)COL,
                (nn_tensor_t*)K, bso, &plan, &job);
  ...
  for (int cout = 0; cout < y_params.channels; cout++)
    for (int row = 0; row < conv2d_window.shape.height; row++)
      for (int col = 0; col < conv2d_window.shape.width; col++)
        for (int cin = 0; cin < x_params.channels; cin++)
          K[cout][row * K_W * CHANS_IN + col * CHANS_IN + cin] =
              cout * (((K_H * K_W * CHANS_IN + 3) >> 2) << 2) +
              row * K_W * CHANS_IN + col * CHANS_IN + cin;
...
void test_conv2d_im2col_case14() {
  nn_tensor_t WORD_ALIGNED
      K[64][CHANS_OUT][(((K_H * K_W * CHANS_IN + 3) >> 2) << 2)];
  nn_image_t WORD_ALIGNED
      COL[((K_H * K_W * CHANS_IN + VPU_INT8_EPV - 1) >> VPU_INT8_EPV_LOG2)
          << VPU_INT8_EPV_LOG2];
  nn_image_t WORD_ALIGNED X[X_HEIGHT][X_WIDTH][CHANS_IN];
...
void test_conv2d_im2col_case16() {
  ...
  nn_conv2d_job_params_t job_params[] = {
      {{1, 0, 0}, {2, 1, 16}},  {{1, 1, 0}, {2, 2, 16}},
      {{0, 0, 0}, {1, 3, 32}},  {{1, 0, 16}, {1, 3, 16}},
      {{2, 0, 16}, {1, 2, 16}},
