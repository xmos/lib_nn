...
void run_bsign_test(int8_t *x, size_t inputLength, int8_t zeroPoint,
                    size_t jobCount) {
  ...
  for (int i = 0; i < outputLength; i++) {
    if (y[i] != y_exp[i])
      sprintf(str_buff, "(jobs: %zu) (inputLen: %zu) (index: %d)", jobCount,
              inputLength, i);
...
void test_bsign_8_basic0() {
  ...
  int8_t WORD_ALIGNED x[VPU_INT8_EPV + 8] = {
      ...
      0x7E, 0x00, 0x80, 0x80, 0x80, 0x80, 0x00, 0x00, 0x00, 0xFF,
...
#define MULT 7
  ...
  int8_t WORD_ALIGNED test_data[VPU_INT8_EPV] = {
      0xFF, 0x01, 0x7F, 0x80, 0xFF, 0x7F, 0x7E, 0x00, 0x80, 0x80, 0x80,
      0x7E, 0x00, 0x00, 0x00, 0xFF, 0xFE, 0x01, 0x7F, 0x80, 0xFF, 0x7F,
      0x7E, 0x00, 0x80, 0x80, 0x80, 0x7E, 0x00, 0x00, 0x00, 0xFF};
