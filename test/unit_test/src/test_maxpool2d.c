...
#define GET_IMAGE_VAL(IMG, PRMS, ROW, COL, CHAN)           \
  (((nn_image_t*)IMG)[ROW * (PRMS.width * PRMS.channels) + \
                      COL * (PRMS.channels) + CHAN])
#define SET_IMAGE_VAL(IMG, PRMS, ROW, COL, CHAN, VAL)     \
  ((nn_image_t*)IMG)[ROW * (PRMS.width * PRMS.channels) + \
                     COL * (PRMS.channels) + CHAN] = VAL
...
void test_maxpool2d_case0() {
  ...
  for (unsigned v = start_case; v < N_casses && v < stop_case; v++) {
    ...
    nn_image_params_t x_params = {casse->X.height, casse->X.width,
                                  casse->channels};
    nn_image_params_t y_params = {casse->Y.height, casse->Y.width,
                                  casse->channels};
          ...
          for (unsigned chn = 0; chn < y_params.channels; chn++) {
            ...
            for (int wr = 0; wr < window_params.shape.height; wr++) {
              for (int wc = 0; wc < window_params.shape.width; wc++) {
                int32_t x_offset =
                    IMG_ADDRESS_VECT(&x_params, (casse->W.vstride * row + wr),
            ...
            Check_Y(y_exp, (nn_image_t*)Y, &y_params, row, col, chn,
                    casse->line);
          }
        }
      }
...
void test_maxpool2d_case1() {
  ...
  typedef struct {
    // struct {        nn_image_vect_t X;  nn_image_vect_t Y;  } start; //choose
    // start locations randomly
    struct {
      nn_image_vect_t shape;
    } output;
    struct {
      struct {
        uint32_t height;
  ...
  for (unsigned v = start_case; v < N_casses && v < stop_case; v++) {
    ...
    nn_image_params_t x_params = {X_HEIGHT, X_WIDTH,
                                  casse->output.shape.channels};
    nn_image_params_t y_params = {casse->output.shape.rows,
                                  casse->output.shape.cols,
                                  casse->output.shape.channels};
    ...
    for (unsigned rep = 0; rep < REPS; rep++) {
      ...
      job_params.size.rows =
          y_params.height == 1
              ? 1
              : (pseudo_rand_uint16() % (y_params.height - 1)) + 1;
      job_params.size.cols =
          y_params.width == 1
              ? 1
              : (pseudo_rand_uint16() % (y_params.width - 1)) + 1;
      job_params.size.channels =
          4 * ((pseudo_rand_uint16() % (y_params.channels / 4 - 1)) + 1);

      // Start indices must be low enough that the job doesn't go outside the
      // image
      ...
      const uint32_t max_chan =
          y_params.channels - job_params.size.channels + 4;
      ...
      for (int out_row = 0; out_row < job_params.size.rows; out_row++) {
        for (int out_col = 0; out_col < job_params.size.cols; out_col++) {
          for (int out_chan = 0; out_chan < job_params.size.channels;
            ...
            struct {
              unsigned row, col;
            } win_start = {job_params.start.rows * window_params.shape.height,
                           job_params.start.cols * window_params.shape.width};

            // The X row is the window start row, plus the output row index (NOT
            // Y row index) times the vertical stride of the window, plus the
            // window row for that output.
            for (int r = 0; r < window_params.shape.height; r++) {
              for (int c = 0; c < window_params.shape.width; c++) {
                struct {
                  unsigned row, col, chn;
                } xdex = {win_start.row +
                              (out_row * window_params.stride.vertical) + r,
                          win_start.col +
                              (out_col * window_params.stride.horizontal) + c,
      ...
      maxpool2d_ext((int8_t*)Y, (int8_t*)X, &x_params, &y_params,
                    &window_params, &job_params, MAXPOOL2D_FLAG_NONE);
      ...
      for (unsigned row = 0; row < y_params.height; row++) {
        for (unsigned col = 0; col < y_params.width; col++) {
          for (unsigned y_chn = 0; y_chn < y_params.channels; y_chn++) {
            ...
            Check_Y(y_exp, (nn_image_t*)Y, &y_params, row, col, y_chn,
                    casse->line);
          }
        }
      }
