...
int8_t pseudo_rand_int8() { return (int8_t)rand(); }

int16_t pseudo_rand_int16() { return (int16_t)rand(); }

uint16_t pseudo_rand_uint16() { return (uint16_t)rand(); }

int32_t pseudo_rand_int32() { return (int32_t)rand(); }

uint32_t pseudo_rand_uint32() { return (uint32_t)rand(); }
...
void pseudo_rand_bytes(char* buffer, unsigned size) {
  ...
  while (size >= sizeof(unsigned)) {
    ...
    for (int i = 0; i < sizeof(unsigned); i++) buffer[b++] = rch[i];
