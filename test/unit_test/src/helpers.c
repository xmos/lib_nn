...
void pick_extreme_bias_post_activation_params(float *post_activation_multiplier,
  ...
  float accu_max =
      receptive_volume *
      2;  // the times 2 is due to the left shift in the output transform.
  ...
  for (unsigned ch = 0; ch < chans_out; ch++) {
    float offset =
        1024. * output_range * (float)pseudo_rand(seed) / (float)INT32_MAX;
    post_activation_multiplier[ch] = output_range / input_range;
    post_activation_bias[ch] =
        output_min - accu_min * output_range / input_range + offset;
  }
}
// TODO pass the clamps
void pick_extreme_mul_post_activation_params(float *post_activation_multiplier,
  ...
  for (unsigned ch = 0; ch < chans_out; ch++) {
    float output_overscale =
        1024. + (float)pseudo_rand(seed) / (float)INT32_MAX;
    float output_range = (output_max - output_min) * output_overscale;
    post_activation_multiplier[ch] = output_range / input_range;
    post_activation_bias[ch] =
        output_min * output_overscale - accu_min * output_range / input_range;
  }
}
...
void pick_post_activation_params(float *post_activation_multiplier,
  ...
  */
  ...
  for (unsigned ch = 0; ch < chans_out; ch++) {
    // Scale the input to extend beyond the range of the output such that we get
    // some outputs that will saturate.
    float output_overscale = 0.5 + (float)pseudo_rand(seed) / (float)INT32_MAX;

    float output_range = (output_max - output_min) * output_overscale;

    // This offset allows the output range to completly miss the int8 output
    // range sometimes
    float offset =
        1.1 * output_range * (float)pseudo_rand(seed) / (float)INT32_MAX;

    post_activation_multiplier[ch] = output_range / input_range;
    post_activation_bias[ch] = output_min * output_overscale -
                               accu_min * output_range / input_range + offset;
  }
}
