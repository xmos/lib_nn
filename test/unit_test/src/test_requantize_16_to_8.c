...
////////////////////////////////////////////////
static void memset16(void* dst, const int16_t val, const unsigned len) {
  for (int i = 0; i < len; i++) {
    ((int16_t*)dst)[i] = val;
  }
}
...
void test_requantize_16_to_8_case0() {
  ...
  int16_t WORD_ALIGNED x[VPU_INT8_ACC_PERIOD] = {
      0x0000, 0x0100, 0x007F, 0x0080, 0x7FFF, 0x7E80, 0x7E7F,
      0x8000, 0x807F, 0x8080, 0x8034, 0,      0,      0};

  int8_t y_exp[VPU_INT8_ACC_PERIOD] = {
      0x00,        0x01,        0x00, 0x01,        0x7F, 0x7F, 0x7E,
      NEG_SAT_VAL, NEG_SAT_VAL, 0x81, NEG_SAT_VAL, 0,    0,    0};
...
void test_requantize_16_to_8_case1() {
  ...
  test_case_t casses[] = {
      // X         //N     //Y
      {0x0000, 16, 0x00},         {0x0F00, 16, 0x0F},   {0x027F, 16, 0x02},
      {0x0280, 16, 0x03},         {-0x0180, 16, -0x01}, {-0x0181, 16, -0x02},
      {0x0100, 1, 0x01},          {0x0100, 8, 0x01},    {0x0100, 15, 0x01},
    ...
    for (unsigned in_place = 0; in_place <= 1; in_place++) {
      ...
      for (int k = 0; k < casse->N; k++) {
        if (dest[k] != casse->exp_y)
          sprintf(str_buff, "(vector: %d) (in-place: %u) (k: %d.)", v, in_place,
                  k);
      ...
      if (!in_place) {
        for (int k = casse->N; k < VEC_LEN; k++) {
          if (dest[k] != casse->exp_y)
            sprintf(str_buff,
                    "Operator didn't respect N. (Vector %d. Element index %d.)",
                    v, k);
  ...
  for (int v = 0; v < REPS; v++) {
    ...
    for (int in_place = 0; in_place < 1; in_place++) {
      ...
      for (int i = 0; i < N; i++) {
        ...
        if (x_orig[i] < -0x7F80) exp_val = NEG_SAT_VAL;

        if (dest[i] != exp_val)
          sprintf(str_buff, "(rep: %d) (N: %u) (index: %d) (x[%d] = %d)", v, N,
                  i, i, x_orig[i]);
      ...
      if (!in_place) {
        for (int i = N; i < MAX_LEN; i++) TEST_ASSERT_EQUAL(XXX, dest[i]);
      }
    }
  }
}
...
void test_requantize_16_to_8_case3() {
  ...
  for (int v = 0; v < REPS; v++) {
    ...
    const unsigned job_start = (pseudo_rand_uint16() % (MAX_LEN + 1)) &
                               0xFFFC;  // must be multiple of 4
    ...
    for (int i = 0; i < MAX_LEN; i++) {
      ...
      if (y[i] != exp_val)
        sprintf(str_buff, "(rep: %d) (N: %u) (index: %d) (x[%d] = %d)", v, N, i,
                i, x_orig[i]);
