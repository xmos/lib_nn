...
#define MIN_CHAN_OUT_GROUPS(CHAN_COUNT)                                     \
  (((CHAN_COUNT + (VPU_INT8_VLMACC_ELMS - 1)) >> VPU_INT8_VLMACC_ELMS_LOG2) \
   << VPU_INT8_VLMACC_ELMS_LOG2)
...
static void check_Y(const int8_t y_exp, const unsigned row, const unsigned col,
                    const unsigned chn, const unsigned line, const int8_t* Y,
...
void test_conv2d_depthwise_case0() {
  ...
  for (unsigned v = start_case; v < N_casses && v < stop_case; v++) {
    ...
    nn_window_params_t conv_window = {
        {K_h, K_w}, {0, 0}, {K_vstride, K_hstride}};

    memset(Y, 0xCC, sizeof(Y));
    conv2d_depthwise((int8_t*)Y, (int8_t*)X, (int8_t*)K, (nn_bso_block_t*)&bso,
                     ZERO_POINT, &x_params, &y_params, &conv_window);
...
void test_conv2d_depthwise_case1() {
  ...
  const test_case_t casses[] = {
      //  X       K       Y
      {0x00, 0x00, 0x00, __LINE__},   {0x01, 0x00, 0x00, __LINE__},
      {0x00, 0x01, 0x00, __LINE__},   {0x01, 0x01, 0x01, __LINE__},
      {-0x01, 0x01, -0x01, __LINE__}, {0x01, -0x01, -0x01, __LINE__},
      {0x02, 0x02, 0x04, __LINE__},   {0x08, 0x04, 0x20, __LINE__},
  };
  ...
  for (unsigned v = start_case; v < N_casses && v < stop_case; v++) {
    ...
    for (int p = 0; p < sizeof(chan_counts) / sizeof(unsigned); p++) {
      ...
      memset(X, casse->x,
             x_params.height * x_params.width * x_params.channels *
                 sizeof(int8_t));
      ...
      TEST_ASSERT_EQUAL(
          (int8_t)0xCC,
          ((int8_t*)
               Y)[IMG_ADDRESS_VECT(&y_params, y_params.height - 1,
                                   y_params.width - 1, y_params.channels - 1) +
                  1]);
    }
  }
}
...
void test_conv2d_depthwise_case2() {
  ...
  const test_case_t casses[] = {
      ...
      {0x01, 0x01, 1, 1, 1, 1, __LINE__}, {0x01, 0x01, 1, 2, 1, 2, __LINE__},
      {0x01, 0x01, 1, 2, 1, 2, __LINE__}, {0x01, 0x01, 2, 2, 2, 2, __LINE__},
      {0x01, 0x01, 1, 3, 1, 3, __LINE__}, {0x01, 0x01, 2, 3, 2, 3, __LINE__},
      {0x01, 0x01, 3, 3, 3, 3, __LINE__}, {0x01, 0x01, 2, 6, 2, 6, __LINE__},
      {0x01, 0x01, 6, 3, 6, 3, __LINE__}, {0x01, 0x01, 6, 6, 6, 6, __LINE__},
      {0x01, 0x01, 4, 4, 4, 4, __LINE__}, {0x01, 0x01, 4, 4, 1, 1, __LINE__},
  ...
  for (unsigned v = start_case; v < N_casses && v < stop_case; v++) {
    ...
    memset(
        X, casse->x,
        x_params.height * x_params.width * x_params.channels * sizeof(int8_t));
    memset(K, casse->k,
           casse->K_h * casse->K_w * y_params.channels * sizeof(int8_t));
    ...
    nn_window_params_t conv_window = {
        {casse->K_h, casse->K_w}, {0, 0}, {casse->v_stride, casse->h_stride}};
...
void test_conv2d_depthwise_case3() {
  ...
  nn_window_params_t conv_window = {
      {K_h, K_w}, {-(K_h / 2), -(K_w / 2)}, {v_stride, h_stride}};
...
void test_conv2d_depthwise_case4() {
  ...
  for (unsigned v = start_case; v < N_casses && v < stop_case; v++) {
    ...
    nn_conv2d_job_params_t job_params = {
        {casse->Y_start.row, casse->Y_start.col, casse->Y_start.channel},
    ...
    memset(Y, 0xCC, sizeof(Y));
    conv2d_depthwise_ext((int8_t*)Y, (int8_t*)X, (int8_t*)K,
                         (nn_bso_block_t*)bso, ZERO_POINT, &x_params, &y_params,
                         &conv_window, &job_params, 0);
    ...
    for (unsigned row = 0; row < y_params.height; row++) {
      for (unsigned col = 0; col < y_params.width; col++) {
        for (unsigned chn = 0; chn < y_params.channels; chn++) {
          ...
          if (row < casse->Y_start.row ||
              row >= casse->Y_start.row + casse->output.rows ||
              col < casse->Y_start.col ||
              col >= casse->Y_start.col + casse->output.cols ||
              chn < casse->Y_start.channel ||
              chn >= casse->Y_start.channel + casse->output.channels) {
            y_exp = 0xCC;
          }
...
#define JOB_COUNT 9
  ...
  nn_conv2d_job_params_t job_params[JOB_COUNT] = {
      {{0, 0, 0}, {2, 1, 16}},  {{0, 1, 0}, {2, 2, 16}},
      {{2, 0, 0}, {2, 3, 16}},  {{4, 0, 0}, {1, 2, 16}},
      {{0, 0, 16}, {4, 3, 16}}, {{4, 0, 16}, {1, 1, 16}},
      {{0, 0, 32}, {5, 1, 4}},  {{0, 1, 32}, {4, 2, 4}},
      {{4, 2, 0}, {1, 1, 36}},  // last block remains uncomputed, and should be
                                // 0xCC
  ...
  for (unsigned row = 0; row < y_params.height; row++) {
    for (unsigned col = 0; col < y_params.width; col++) {
      for (unsigned chn = 0; chn < y_params.channels; chn++) {
        ...
        if ((row == 4 && col == 1 && chn >= 16)) y_exp = 0xCC;
...
void test_conv2d_depthwise_case6_() {
  ...
  */
  ...
  int8_t Y_exp[Y_HEIGHT][Y_WIDTH] = {
      {0x1D, 0x15, 0x15, 0x15, 0x15}, {0x15, 0x09, 0x09, 0x09, 0x09},
      {0x15, 0x09, 0x09, 0x09, 0x09}, {0x15, 0x09, 0x09, 0x09, 0x09},
...
void test_conv2d_depthwise_case6() {
  ...
  int8_t Y_exp[Y_HEIGHT][Y_WIDTH][CHANNELS] = {{{-100, -19, -38, 6}}};
...
void test_conv2d_depthwise_case8() {
  ...
  memset(Y, 0xCC, sizeof(Y));
  conv2d_depthwise((nn_image_t*)Y, (nn_image_t*)X, (nn_tensor_t*)K,
                   (nn_bso_block_t*)bso, ZERO_POINT, &x_params, &y_params,
                   &conv_window);
...
void test_conv2d_depthwise_case9() {
  ...
  for (int k = K_c; k < CHANNELS; k++) Y_exp[k] = 0xCC;
  ...
  memset(Y, 0xCC, sizeof(Y));
  conv2d_depthwise_ext((nn_image_t*)Y, (nn_image_t*)X, (nn_tensor_t*)K,
                       (nn_bso_block_t*)bso, ZERO_POINT, &x_params, &y_params,
