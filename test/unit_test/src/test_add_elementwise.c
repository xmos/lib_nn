...
static void test_add_elementwise_case2() {
  ...
  for (int v = 0; v < REPS; v++) {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      min += (((int32_t)-128) << (-params.input[i].shr)) *
             params.input[i].multiplier;
      max += (((int32_t)127) << (-params.input[i].shr)) *
             params.input[i].multiplier;
    }
    ...
    for (int i = elm_start; i < elm_start + elm_count; i++) {
      ...
      Y_expected[i] =
          vlsat_single_s8(acc, params.output.shr, NEG_SAT_VAL, VPU_INT8_MAX);
    }
