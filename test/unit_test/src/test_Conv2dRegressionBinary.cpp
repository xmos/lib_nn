...
BinaryKernelStimulus create_simple_binary_stimulus(Filter2dGeometry &geom) {
  ...
  for (int ch = 0; ch < geom.output.depth; ch++) {
    ks.eff_mult[ch] = rng.rand<float>(0.5, 2.0) * 256. / (float)receptive_volume;
      ...
      for (int x_channels = 32; x_channels <= 32*3; x_channels += 32) {
        for (int k_height = 1; k_height <= x_height; ++k_height) {
          for (int k_width = 1; k_width <= x_width; ++k_width) {
            for (int k_depth = 32; k_depth <= 32*10; k_depth += 32) {
                          ...
                          for (int right_pad = 0; right_pad <= 0; ++right_pad) {
                            for (int bottom_pad = 0; bottom_pad <= 0;
                              ...
                              for (int itt = 0; itt < itt_count; ++itt) {
                                ...
                                auto expected = 
                                    nn::test::ops::ref::Conv2dBNNBinaryOutReference(
                                      geom, input.data(), 
                                      weights.data(), 
                                      thresholds.data());

                                //this is the value that would be inserted if we were padding
                                ...
                                // over reads
                                threshold_t pad_val =
                                    rng.rand<threshold_t>();  // this is arbitrary
                          ...
                          for (int right_pad = 0; right_pad <= 0; ++right_pad) {
                            for (int bottom_pad = 0; bottom_pad <= 0;
                              ...
                              for (int itt = 0; itt < itt_count; ++itt) {
                                ...
                                MatMulBinaryDirectFn::Params p(X, K, x_channels);
                                ...
                                BNNConv2dValidDirectBinary conv2d(&akp, &memcpy,
                                                           &aggregator, &ot);
                                ...
                                conv2d.execute((int8_t*)&output[0], (int8_t*)&input[0]);
                          ...
                          for (int right_pad = 0; right_pad <= 0; ++right_pad) {
                            for (int bottom_pad = 0; bottom_pad <= 0;
                              ...
                              for (int itt = 0; itt < itt_count; ++itt) {
                                ...
                                auto expected = 
                                    nn::test::ops::ref::Conv2dBNNIntOutReference(
                                      geom, input.data(), 
                                      weights.data(), 
                                ...
                                int receptive_volume = k_height * k_width * x_channels;
                                ...
                                auto accu_overlaps = OT_int8_clamped::get_accumulator_overlaps(
                                ...
                                auto serialised_offsets_multipliers_and_biases = 
                                  OutputTransformFn::serialise_memory(accu_overlaps, qp.multipliers, qp.biases);
                                ...
                                threshold_t pad_val =
                                    rng.rand<threshold_t>();  // this is arbitrary
                                OutputTransformFn::pad_final_access(
                                    serialised_offsets_multipliers_and_biases, 16,
                                    pad_val);

                                OT_int8_clamped::Params ot_params((int32_t)k_depth,
                                                          qp.initial_shr,
                                                          qp.final_shr);
                                ...
                                BNNConv2dValidIndirectInt8 conv2d(&akp, &memcpy,
                                                            &aggregator, &ot);
                                alignas(4)
                          ...
                          for (int right_pad = 0; right_pad <= 0; ++right_pad) {
                            for (int bottom_pad = 0; bottom_pad <= 0;
                              ...
                              for (int itt = 0; itt < itt_count; ++itt) {
                                ...
                                BNNConv2dValidDirectInt8 conv2d(&akp, &memcpy,
                                                           &aggregator, &ot);
                                ...
                                conv2d.execute((int8_t*)output, (int8_t*)input.data());
