...
void impl_vpu_memset_32_pseudo_random(
    int pointer_inc, 
    int set_words_inc,
...
void test_vpu_memset_vector_directed_0(){
    ...
    int seed = 69;             
    int32_t value = (int32_t)pseudo_rand(&seed);
    memset(dst, 0, (VPU_MEMSET_VECTOR_WORDS *  DIR_TEST_0_VECTORS)*sizeof(int32_t));

    vpu_memset_vector(dst, value, DIR_TEST_0_VECTORS);

    TEST_ASSERT_EACH_EQUAL_INT32(value, dst, (VPU_MEMSET_VECTOR_WORDS *  DIR_TEST_0_VECTORS));
}

void impl_vpu_memset_vector_pseudo_random(
    int pointer_inc, 
    int set_vectors_inc,
    ...
    int32_t * dst = (int32_t *)malloc(bytes_per_vpu_vector * max_test_vpu_vectors + 1);
    ...
    for(int dst_offset = 0; dst_offset < mem_words - VPU_MEMSET_VECTOR_WORDS; dst_offset += pointer_inc){

        int max_set_vectors = (mem_words - dst_offset) / VPU_MEMSET_VECTOR_WORDS;

        for(int set_vectors = 1; set_vectors < max_set_vectors; set_vectors += set_vectors_inc){
