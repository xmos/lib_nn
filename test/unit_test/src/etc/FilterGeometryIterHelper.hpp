...
static inline nn::ff::FilterGeometryIterator SimpleDepthwise(
  ...
  return FilterGeometryIterator(
      nn::Filter2dGeometry(
          nn::ImageGeometry{output_spatial_range[0], output_spatial_range[0],
                            channel_count_range[0]},
          nn::ImageGeometry{1, 1, 4},
          nn::WindowGeometry(std::array<int, 3>{{window_spatial_range[0],
                                                window_spatial_range[0], 1}},
                             std::array<int, 2>{{0, 0}},
                             std::array<int, 3>{{0, 0, 1}},
                             std::array<int, 2>{{1, 1}},
                             1)),
      {new OutputShape(std::array<int, 3>{{output_spatial_range[0],
                                          output_spatial_range[0],
                                          channel_count_range[0]}},
                       std::array<int, 3>{{output_spatial_range[1],
                                          output_spatial_range[1],
                                          channel_count_range[1]}},
                       std::array<int, 3>{{output_step, output_step,
                                          channel_step}}),
       new WindowShape(std::array<int, 2>{{window_spatial_range[0],
                                          window_spatial_range[0]}},
                       std::array<int, 2>{{window_spatial_range[1],
                                          window_spatial_range[1]}},
                       std::array<int, 2>{{window_step, window_step}}),
