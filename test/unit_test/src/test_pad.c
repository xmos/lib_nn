...
void impl_pad_param_space(const unsigned channels_per_input_word,
  ...
  for (unsigned pad_val_idx = 0; pad_val_idx < 8; pad_val_idx++) {
    ...
    for (unsigned x_height = 1; x_height <= max_x_height; ++x_height) {
      for (unsigned x_width = 1; x_width <= max_x_width; ++x_width) {
        for (unsigned x_chan_words = x_channel_inc;
             x_chan_words <= max_x_chan_words; x_chan_words += x_channel_inc) {
          size_t X_bytes =
              sizeof_input_word * x_height * x_width * x_chan_words;
            ...
            for (unsigned pad_bottom = 0; pad_bottom <= max_pad_bottom;
              ...
              for (unsigned pad_right = 0; pad_right <= max_pad_right;
                ...
                for (unsigned pad_left = 0; pad_left <= max_pad_left;
                  ...
                  size_t Y_bytes =
                      sizeof_input_word * y_height * y_width * x_chan_words;
                  ...
                  pad_ref(Y_ref, X, &p, &xp, bytes_per_pixel, pad_value);
