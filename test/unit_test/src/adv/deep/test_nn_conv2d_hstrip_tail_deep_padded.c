...
static void check_Y(const int8_t y_exp, const unsigned row, const unsigned col,
                    const unsigned chn, const unsigned line,
                    const nn_image_t* Y, const nn_image_params_t* y_params) {
  char str_buff[200];
  ...
  if (y != y_exp) {
    sprintf(str_buff, "(row, col, chn) = (%u, %u, %u)  [test vector @ %u]", row,
            col, chn, line);
  }
...
#define K_hstride (1)
#define ZERO_POINT (0)
void test_nn_conv2d_hstrip_tail_deep_padded_case0() {
  PRINTF("%s...\n", __func__);
  ...
  for (int v = start_case; v < N_casses && v <= stop_case; v++) {
    ...
    for (int C_out = 4; C_out < CHANS_OUT_MAX; C_out += 4) {
      ...
      memset(
          K, casse->k,
          y_params.channels * K_h * K_w * x_params.channels * sizeof(int8_t));
      ...
      nn_conv2d_hstrip_tail_deep_padded(
          (nn_image_t*)Y, (nn_image_t*)X, K_init, (nn_bso_block_t*)&bso, K_h,
          K_w, K_hstride, x_params.channels, 0, 0, 0, 0,
          (x_params.width - K_w) * x_params.channels, k_cout_stride,
          y_params.channels, 1, zero_point_vec, C_out);
      ...
      for (unsigned row = 0; row < y_params.height; row++) {
        for (unsigned col = 0; col < y_params.width; col++) {
          for (unsigned chn = 0; chn < y_params.channels; chn++) {
            ...
            check_Y(y_exp, row, col, chn, casse->line, (nn_image_t*)Y,
                    &y_params);
          }
        }
      }
...
#define K_w (1)
#define K_hstride (1)
void test_nn_conv2d_hstrip_tail_deep_padded_case1() {
  PRINTF("%s...\n", __func__);
  ...
  for (int v = start_case; v < N_casses && v <= stop_case; v++) {
    ...
    for (int C_out = 4; C_out < CHANS_OUT_MAX; C_out += 4) {
      ...
      for (int pad_t = 0; pad_t < 3; pad_t++) {
        ...
        memset(Y, 0xCC, sizeof(Y));
        nn_conv2d_hstrip_tail_deep_padded(
            (nn_image_t*)Y, X_patch_start, K_init, (nn_bso_block_t*)&bso, K_h,
            K_w, K_hstride, x_params.channels, pad_t, pad_b, pad_l, pad_r,
            (x_params.width - K_w) * x_params.channels, k_cout_stride,
            y_params.channels, 1, zero_point_vec, C_out);
...
#define K_w (3)
#define K_hstride (1)
void test_nn_conv2d_hstrip_tail_deep_padded_case2() {
  PRINTF("%s...\n", __func__);
...
#define K_w (3)
#define K_hstride (1)
void test_nn_conv2d_hstrip_tail_deep_padded_case3() {
  PRINTF("%s...\n", __func__);
...
#define K_hstride (1)
#define ZERO_POINT (0)
void test_nn_conv2d_hstrip_tail_deep_padded_case4() {
  PRINTF("%s...\n", __func__);
  ...
  for (int v = start_case; v < N_casses && v <= stop_case; v++) {
    ...
    for (int C_out = 4; C_out < CHANS_OUT_MAX; C_out += 4) {
      ...
      nn_conv2d_hstrip_tail_deep_padded(
          ...
          K_w, K_hstride, x_params.channels, pad_t, pad_b, pad_l, pad_r,
          (x_params.width - K_w) * x_params.channels, k_cout_stride,
          y_params.channels, Y_WIDTH, zero_point_vec, C_out);
...
#define K_w (3)
#define K_hstride (1)
void test_nn_conv2d_hstrip_tail_deep_padded_case5() {
  PRINTF("%s...\n", __func__);
  ...
  test_case_t casses[] = {
      ...
      {0, 0, 0, 6, __LINE__},   {4, 0, 0, 6, __LINE__},
      {0, 4, 0, 6, __LINE__},   {4, 4, 0, 6, __LINE__},
      {8, 4, 0, 7, __LINE__},   {4, 8, 0, 7, __LINE__},
      {-8, 4, 0, 7, __LINE__},  {4, -8, 0, 7, __LINE__},
      {0, 0, 4, 0, __LINE__},   {4, 0, 4, 0, __LINE__},
      {0, 4, 4, 6, __LINE__},   {4, 4, 4, 6, __LINE__},
      {-4, 4, 8, 6, __LINE__},  {-8, 4, 8, 8, __LINE__},
      {-16, 4, 8, 9, __LINE__},
  };
  ...
  for (int v = start_case; v < N_casses && v <= stop_case; v++) {
    ...
    for (int C_out = 4; C_out < CHANS_OUT_MAX; C_out += 4) {
      ...
      Y_exp[0][0] =
          ((CHANS_IN * casse->k *
            (9 * ((int32_t)casse->x) + 0 * ((int32_t)casse->zero_point))) +
           (1 << (casse->shift1 - 1))) >>
          casse->shift1;
      Y_exp[0][1] =
          ((CHANS_IN * casse->k *
            (6 * ((int32_t)casse->x) + 3 * ((int32_t)casse->zero_point))) +
           (1 << (casse->shift1 - 1))) >>
          casse->shift1;
      Y_exp[0][2] =
          ((CHANS_IN * casse->k *
            (3 * ((int32_t)casse->x) + 6 * ((int32_t)casse->zero_point))) +
           (1 << (casse->shift1 - 1))) >>
          casse->shift1;
...
#define K_w (1)
#define K_hstride (1)
void test_nn_conv2d_hstrip_tail_deep_padded_case6() {
  PRINTF("%s...\n", __func__);
  ...
  memset(Y, 0xCC, sizeof(Y));
  nn_conv2d_hstrip_tail_deep_padded(
      (nn_image_t*)Y, X_patch_start, K_init, (nn_bso_block_t*)&bso, K_h, K_w,
      K_hstride, x_params.channels, pad_t, pad_b, pad_l, pad_r,
      (x_params.width - K_w) * x_params.channels, k_cout_stride,
      y_params.channels, Y_WIDTH, zero_point_vec, CHANS_OUT);
...
void test_nn_conv2d_hstrip_tail_deep_padded_case7() {
  ...
  memset(Y, 0xCC, sizeof(Y));
  nn_conv2d_hstrip_tail_deep_padded(
      &Y[0][0][16], X_patch_start, K_init, (nn_bso_block_t*)&bso, K_h, K_w,
      K_hstride, x_params.channels, pad_t, pad_b, pad_l, pad_r,
      (x_params.width - K_w) * x_params.channels, k_cout_stride,
      y_params.channels, Y_WIDTH, zero_point_vec, y_params.channels % 16);
...
void test_nn_conv2d_hstrip_tail_deep_padded() {
  ...
  RUN_TEST(test_nn_conv2d_hstrip_tail_deep_padded_case0);
  RUN_TEST(test_nn_conv2d_hstrip_tail_deep_padded_case1);
  RUN_TEST(test_nn_conv2d_hstrip_tail_deep_padded_case2);
  RUN_TEST(test_nn_conv2d_hstrip_tail_deep_padded_case3);
