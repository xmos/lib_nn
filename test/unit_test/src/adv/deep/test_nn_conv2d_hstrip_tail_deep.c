...
#define K_w (1)
#define K_hstride (1)
void test_nn_conv2d_hstrip_tail_deep_case0() {
  PRINTF("%s...\n", __func__);
  ...
  for (int v = start_case; v < N_casses && v <= stop_case; v++) {
    ...
    for (int C_out = 4; C_out < CHANS_OUT_MAX; C_out += 4) {
      ...
      nn_conv2d_hstrip_tail_deep((nn_image_t*)Y, (nn_image_t*)X, K_init,
                                 (nn_bso_block_t*)&bso, K_h, K_w, K_hstride,
                                 x_params.channels,
                                 (x_params.width - K_w) * x_params.channels,
                                 k_cout_stride, y_params.channels, 1, C_out);
...
#define K_w (2)
#define K_hstride (2)
void test_nn_conv2d_hstrip_tail_deep_case1() {
  PRINTF("%s...\n", __func__);
  ...
  for (int C_out = 4; C_out < CHANS_OUT_MAX; C_out += 4) {
    ...
    nn_conv2d_hstrip_tail_deep(
        (nn_image_t*)Y, (nn_image_t*)X, K_init, (nn_bso_block_t*)&bso, K_h, K_w,
        K_hstride, x_params.channels,
    ...
    for (unsigned row = 0; row < y_params.height; row++) {
      for (unsigned col = 0; col < y_params.width; col++) {
        for (unsigned chn = 0; chn < y_params.channels; chn++) {
          ...
          check_Y(y_expected, row, col, chn, __LINE__, (nn_image_t*)Y,
                  &y_params);
        }
      }
    }
...
#define K_w (2)
#define K_hstride (2)
void test_nn_conv2d_hstrip_tail_deep_case2() {
  PRINTF("%s...\n", __func__);
  ...
  for (int C_out = 4; C_out < CHANS_OUT_MAX; C_out += 4) {
    ...
    for (int k = 0; k < y_params.channels; k++) {
      ...
      BSO.scale[k] = 64;
      BSO.shift2[k] = 6;
      BSO.offset_scale[k] = 1 << BSO.shift2[k];
      BSO.offset[k] = -k;
    }
...
#define K_w (1)
#define K_hstride (1)
void test_nn_conv2d_hstrip_tail_deep_case3() {
  PRINTF("%s...\n", __func__);
  ...
  nn_conv2d_hstrip_tail_deep(
      (nn_image_t*)Y, (nn_image_t*)X, K_init, (nn_bso_block_t*)&bso, K_h, K_w,
      K_hstride, x_params.channels, (x_params.width - K_w) * x_params.channels,
      k_cout_stride, y_params.channels, Y_WIDTH, y_params.channels);
...
void test_nn_conv2d_hstrip_tail_deep_case4() {
  ...
  memset(Y, 0xCC, sizeof(Y));
  nn_conv2d_hstrip_tail_deep(
      &Y[0][0][16], (nn_image_t*)X, K_init, (nn_bso_block_t*)&bso, K_h, K_w,
      K_hstride, x_params.channels, (x_params.width - K_w) * x_params.channels,
      k_cout_stride, y_params.channels, Y_WIDTH, y_params.channels % 16);
...
void test_nn_conv2d_hstrip_tail_deep() {
  ...
  RUN_TEST(test_nn_conv2d_hstrip_tail_deep_case0);
