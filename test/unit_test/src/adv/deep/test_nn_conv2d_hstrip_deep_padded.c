...
void test_nn_conv2d_hstrip_deep_padded_case0() {
  ...
  for (int v = start_case; v < N_casses && v <= stop_case; v++) {
    ...
    nn_conv2d_hstrip_deep_padded(
        (nn_image_t*)Y, (nn_image_t*)X, KERNEL_4D_COG_LAST_CHAN_START(K, 0),
        (nn_bso_block_t*)&bso, K_h, K_w, K_hstride, x_params.channels, 0, 0, 0,
        0, (x_params.width - K_w) * x_params.channels,
...
void test_nn_conv2d_hstrip_deep_padded_case1() {
  ...
  for (int v = start_case; v < N_casses && v <= stop_case; v++) {
    ...
    for (int pad_t = 0; pad_t < 3; pad_t++) {
      ...
      memset(Y, 0xCC, sizeof(Y));
      nn_conv2d_hstrip_deep_padded(
          (nn_image_t*)Y, X_patch_start, KERNEL_4D_COG_LAST_CHAN_START(K, 0),
          (nn_bso_block_t*)&bso, K_h, K_w, K_hstride, x_params.channels, pad_t,
          pad_b, pad_l, pad_r, (x_params.width - K_w) * x_params.channels,
          -K_h * K_w * x_params.channels, y_params.channels, 1, zero_point_vec);
    ...
    for (int pad_t = 1; pad_t < K_h; pad_t++) {
      ...
      for (int pad_l = 0; pad_l < K_w; pad_l++) {
        ...
        nn_conv2d_hstrip_deep_padded(
            ...
            (x_params.width - K_w) * x_params.channels,
            -K_h * K_w * x_params.channels, y_params.channels, 1,
            zero_point_vec);
...
void test_nn_conv2d_hstrip_deep_padded_case4() {
  ...
  for (int v = start_case; v < N_casses && v <= stop_case; v++) {
    ...
    nn_conv2d_hstrip_deep_padded(
        ...
        (nn_bso_block_t*)&bso, K_h, K_w, K_hstride, x_params.channels, pad_t,
        pad_b, pad_l, pad_r, (x_params.width - K_w) * x_params.channels,
        -K_h * K_w * x_params.channels, y_params.channels, Y_WIDTH,
        zero_point_vec);
