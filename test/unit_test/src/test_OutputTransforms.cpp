...
/*
  Given a number of coefficients this generates realistic accumulator min and max values.
*/
void pick_representitive_accu_bounds(int coef_count, int &accu_min, int &accu_max){
    accu_min = 0;
    accu_max = 0;
  ...
  for (int output_ch_count = 4; output_ch_count <= 64; output_ch_count+=4) {
      for(int itt=0;itt < 32;itt++){
        ...
        for(int ch=0; ch< output_ch_count;++ch){
          ...
          float multiplier = product_target / (float)std::max(-accu_min, accu_max);

          //the bias must be between [-2**(N+1) - 1, 2**(N+1)]
          double bias = (double)get_random_uniform_from_range(bias_low, bias_high);

          OutputTransformFn::ActivationParams a(bias, multiplier, accu_min, accu_max);
        ...
        // this is to work around array over reads - padding wont effect the result.
        ...
        for (int ocg = 0; ocg < ocg_count; ++ocg) {
          ...
          for (int t = 0; t < 1<<6; t++) {
            ...
            for (int output_chan = 0; output_chan < chs_in_group; ++output_chan) {
              ...
              double expected = (double)accu_values[output_chan] * 
              ...
              //     std::cout << ch << " accu_min: " << mul_and_biases[ch].original_accu_min_val << " accu_max:" 
              ...
              //   std::cout << "actual_output_channel " << actual_output_channel <<std::endl;
              //   std::cout << "accu_values[output_chan] " << accu_values[output_chan] <<std::endl;
...
*/
void test_small_range(const int accu_min, const int accu_max, const int const_bias) {
  ...
  for (int output_ch_count = 4; output_ch_count <= 64; output_ch_count+=4) {
      ...
      for(int ch=0; ch< output_ch_count;++ch){
        OutputTransformFn::ActivationParams a(const_bias, 1.0, accu_min, accu_max);
        mul_and_biases.push_back(a);
      }
      ...
      for (int ocg = 0; ocg < ocg_count; ++ocg) {
        ...
        for (int t = accu_min; t <= accu_max; t++) {
          ...
          for (int output_chan = 0; output_chan < chs_in_group; ++output_chan) {
            ...
            double expected = (float)t * mul_and_biases[actual_output_channel].original_multiplier +
                              mul_and_biases[actual_output_channel].original_bias;
            expected = std::round(std::min(
                std::max(expected, (double)INT8_MIN), (double)INT8_MAX));
...
void Test_OT_int8_range(){
  test_small_range(INT8_MIN, INT8_MAX, 0);
}
