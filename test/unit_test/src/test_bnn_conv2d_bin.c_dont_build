...
static const int undef_word = (undef_sentinel << 24) + (undef_sentinel << 16) +
                              (undef_sentinel << 8) + undef_sentinel;
/*
...
static void run_bin_config(bnn_b32_t* Y_p, bnn_b32_t* Y_ref_p, bnn_b32_t* X_ref,
                           ...
                           int32_t* thresholds_ref, int32_t* thresholds_p,

                           int* chan_overlaps,

                           unsigned x_height, unsigned x_width,
                           unsigned k_height, unsigned k_width,
                           unsigned chans_in, unsigned chans_out,
                           unsigned h_stride, unsigned v_stride, int seed,
                           unsigned y_loc_channel, unsigned y_sub_channel,
                           void (*impl_fn)()) {
  // printf("h_stride:%u v_stride:%u k_height:%u k_width:%u x_height:%u
  // x_width:%u chans_in:%u chans_out:%u seed:%d y_loc_channel:%d
  // y_sub_channel:%d\n",
  //   h_stride, v_stride, k_height, k_width, x_height, x_width, chans_in,
  //   chans_out, seed, y_loc_channel, y_sub_channel);
  ...
  pick_threshold_params(thresholds_ref, chans_out,
                        chans_in * k_height * k_width);
  ...
  bnn_reorder_kernel_tensor((bnn_b32_t*)K_p, (bnn_b32_t*)K_ref_p, k_height,
                            k_width, chans_in, chans_out, chan_overlaps);

  bnn_reorder_threshold_tensor(thresholds_p, thresholds_ref, chans_out,
                               k_width * k_height * chans_in, chan_overlaps);

  impl_fn(Y_p, X_ref, K_p, thresholds_p, &x, &y, &k, y_loc_channel,
          y_sub_channel);
  ...
  for (unsigned h = 0; h < y.height; h++) {
    for (unsigned w = 0; w < y.width; w++) {
      ...
      for (unsigned c = chan_b32_start; c < chan_b32_start + chan_b32_count;
           c++) {
        // printf("b\n");
        TEST_ASSERT_EQUAL_INT(Y_ref[h][w][c], Y[h][w][c]);
      }
      for (unsigned c = chan_b32_start + chan_b32_count; c < chan_b32_out;
           c++) {
        // printf("c\n");
        TEST_ASSERT_EQUAL_INT(undef_word, Y[h][w][c]);
      }
              ...
              for (unsigned chans_in = min_chans_in; chans_in <= max_chans_in;
                ...
                for (unsigned chans_out = min_chans_out;
                  ...
                  bnn_b32_t* K = (bnn_b32_t*)malloc(
                      K_ref_bytes + sizeof(bnn_b32_t) * K_OVERREAD_WORDS);

                  size_t X_ref_bytes =
                      sizeof(bnn_b32_t) * (x_height * x_width * chan_words_in +
                                           X_REF_OVERREAD_WORDS);
                  bnn_b32_t* X_ref = (bnn_b32_t*)malloc(X_ref_bytes);
                  int32_t* thresholds = (int32_t*)malloc(
                      sizeof(int32_t) * (chans_out + (16 - chans_out % 16)));

                  int32_t* thresholds_ref =
                      (int32_t*)malloc(sizeof(int32_t) * chans_out);
                  int* chan_overlaps = (int*)malloc(sizeof(int) * (chans_out));

                  bnn_b32_t* Y = (bnn_b32_t*)malloc(
                      sizeof(bnn_b32_t) * y_height * y_width * chans_out / 32);
                  bnn_b32_t* Y_ref = (bnn_b32_t*)malloc(
                      sizeof(bnn_b32_t) * y_height * y_width * chans_out / 32);
                  ...
                  for (unsigned c = 0; c < 1 << 1; c++) {
                    ...
                    for (unsigned y_loc_channel = 0;
                         y_loc_channel < chans_out - chans_out_inc;
                         y_loc_channel += chans_out_inc) {
                      for (unsigned y_sub_channel = chans_out_inc;
                           y_sub_channel <= chans_out - y_loc_channel;
                           y_sub_channel += chans_out_inc) {
                        run_bin_config(
                            ...
                            seed, y_loc_channel, y_sub_channel, valid_impl);
                      }
                    }
                  }
...
void impl_bconv2d_bin_DI_pseudo_random2(const unsigned max_x_height,
    ...
    for (unsigned x_width = 1; x_width <= max_x_width; ++x_width) {
      ...
      for (unsigned chans_out = min_chans_out; chans_out <= max_chans_out;
        ...
        size_t K_ref_bytes = sizeof(bnn_b32_t) *
                             (chans_out * k_height * k_width * chan_words_in);
        bnn_b32_t* K_ref = (bnn_b32_t*)malloc(K_ref_bytes);
        bnn_b32_t* K = (bnn_b32_t*)malloc(K_ref_bytes +
                                          sizeof(bnn_b32_t) * K_OVERREAD_WORDS);

        size_t X_ref_bytes =
            sizeof(bnn_b32_t) *
            (x_height * x_width * chan_words_in + X_REF_OVERREAD_WORDS);
        bnn_b32_t* X_ref = (bnn_b32_t*)malloc(X_ref_bytes);
        int32_t* thresholds_ref = (int32_t*)malloc(
            sizeof(int32_t) * (chans_out + (16 - chans_out % 16)));

        int32_t* thresholds = (int32_t*)malloc(sizeof(int32_t) * chans_out);
        int* chan_overlaps = (int*)malloc(sizeof(int) * (chans_out));

        bnn_b32_t* Y = (bnn_b32_t*)malloc(sizeof(bnn_b32_t) * y_height *
                                          y_width * chans_out / 32);
        bnn_b32_t* Y_ref = (bnn_b32_t*)malloc(sizeof(bnn_b32_t) * y_height *
                                              y_width * chans_out / 32);
        ...
        for (unsigned y_loc_channel = 0; y_loc_channel < chans_out;
          ...
          for (unsigned ch_group_count = 0; ch_group_count < channel_groups;
            ...
            run_bin_config((bnn_b32_t*)Y, (bnn_b32_t*)Y_ref, (bnn_b32_t*)X_ref,
                           ...
                           (int32_t*)thresholds_ref, (int32_t*)thresholds,
                           (int*)chan_overlaps, x_height, x_width, k_height,
                           k_width, chans_in, chans_out, 1, 1, seed,
...
static void run_bin_sub_image(bnn_b32_t* Y_p, const bnn_b32_t* Y_ref_p,
                              ...
                              const nn_window_params_t* k, unsigned y_loc_width,
                              unsigned y_loc_height, unsigned y_sub_width,
                              unsigned y_sub_height, unsigned y_loc_channel,
                              unsigned y_sub_channel, void (*valid_impl)()) {
  valid_impl(Y_p, X_p, K_p, thresholds, x, y, k, y_loc_width, y_loc_height,
      ...
      for (unsigned h_stride = min_h_stride; h_stride <= max_h_stride;
        ...
        for (unsigned v_stride = min_v_stride; v_stride <= max_v_stride;
          ...
          size_t addressable_Y_bytes =
              sizeof(bnn_b32_t) * y.height * y.width * y.channels / 32;
          ...
          for (unsigned i = 0; i < 1 << 6; i++) {
            ...
            pick_threshold_params(thresholds_ref, chans_out,
                                  chans_in * full_k_height * full_k_width);

            larq_ref_bconv2d_bin_out(&x, &y, &k, (int32_t*)X_ref,
                                     (int32_t*)K_ref, (int32_t*)Y_ref,
                                     (const int32_t*)thresholds_ref);

            // bnn_reorder_int8_kernel_tensor(K_p, K_ref_p, k_height, k_width,
            // chans_in,
            //                           chans_out, chan_overlaps);

            bnn_reorder_kernel_tensor((bnn_b32_t*)K, (bnn_b32_t*)K_ref,
                                      full_k_height, full_k_width, chans_in,
                                      chans_out, chan_overlaps);

            bnn_reorder_threshold_tensor(
                thresholds, thresholds_ref, chans_out,
                  ...
                  for (unsigned y_sub_height = 1;
                    ...
                    for (unsigned y_loc_channel = 0;
                      ...
                      for (unsigned y_sub_channel = chans_out_inc;
                        ...
                        run_bin_sub_image(
                            (bnn_b32_t*)Y, (const bnn_b32_t*)Y_ref,
                            (const bnn_b32_t*)X_ref, (const bnn_b32_t*)K,

                            thresholds,

                            &x, &y, &k, y_loc_width, y_loc_height, y_sub_width,
...
static void generic_kernel_subregion(
  ...
  bconv2d_bin_valid(Y_p, X_p, K_p, thresholds, data_scratch, x, y, k,
...
static void DI_kernel_subregion(
    ...
    unsigned y_sub_width, unsigned y_sub_height, unsigned y_loc_channel,
    unsigned y_sub_channel) {
  bconv2d_bin_DI_valid(Y_p, (const bnn_b256_t*)X_p, (const bnn_b256_t*)K_p,
                       thresholds, x, y, k, y_loc_width, y_loc_height,
                       y_sub_width, y_sub_height, y_loc_channel, y_sub_channel);
}

static void generic_kernel_full_image(bnn_b32_t* Y_p, const bnn_b32_t* X_p,
                                      const bnn_b32_t* K_p,
                                      ...
                                      unsigned y_loc_channel,
                                      unsigned y_sub_channel) {
  bnn_b32_t* data_scratch = (bnn_b32_t*)malloc(
      sizeof(bnn_b32_t) * (k->shape.height * k->shape.width * x->channels / 32 +
                           DATA_SCRATCH_OVERREADWRITE_WORDS));

  bconv2d_bin(Y_p, X_p, K_p, thresholds, data_scratch, x, y, k, 0, 0, y->width,
              y->height, 0, 0, y_loc_channel, y_sub_channel);
  free(data_scratch);
}

static void DI_kernel_full_image(bnn_b32_t* Y_p, const bnn_b32_t* X_p,
                                 const bnn_b32_t* K_p,
                                 ...
                                 unsigned y_loc_channel,
                                 unsigned y_sub_channel) {
  bconv2d_bin_DI(Y_p, (const bnn_b256_t*)X_p, (const bnn_b256_t*)K_p,
                 thresholds, x, y, k, 0, 0, y->width, y->height, 0, 0,
                 y_loc_channel, y_sub_channel);
}

void test_bconv2d_bin_pseudo_random() {
  impl_bconv2d_bin_DI_pseudo_random(1, 5, 1, 5, 32 * 1, 32 * 9, 32 * 1, 32 * 5,
                                    32, 32, 1, 3, 1, 3,
                                    (void*)&generic_kernel_full_image);
}

void test_bconv2d_bin_DI_pseudo_random() {
  impl_bconv2d_bin_DI_pseudo_random(1, 4, 1, 4, 256 * 1, 256 * 2, 32 * 1,
                                    32 * 5, 256, 32, 1, 3, 1, 3,
...
void test_bconv2d_bin_sub_image() {
  impl_bconv2d_bin_sub_image(5, 5, 3, 3, 32 * 1, 32 * 9, 32 * 1, 32 * 5, 32, 32,
                             1, 3, 1, 3, (void*)&generic_kernel_subregion);
}

void test_bconv2d_bin_DI_sub_image() {
  impl_bconv2d_bin_sub_image(5, 5, 3, 3, 256 * 1, 256 * 3, 32 * 1, 32 * 5, 256,
                             32, 1, 3, 1, 3, (void*)&DI_kernel_subregion);
}
